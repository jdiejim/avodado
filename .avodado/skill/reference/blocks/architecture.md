...
edges:
  - crawler -> indexer: pages
  - indexer -> idx
  - qapi --> idx: reads
```
Edges use the terse `from -> to: label` form by default (`-->` dashed,
...
```
`variant: be` is the same engine framed for the backend — use it to draw the
...
- `block` uses **identical YAML** across every `preset` — the preset only
  changes the colored tag pill and section eyebrow. Pick the preset that best
...
- A CI/CD pipeline / DAG is `flow` with `variant: dag` — see `flows.md`.
