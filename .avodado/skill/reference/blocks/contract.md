...
strict field contract for all 94 blocks. Schemas reject unknown fields — use
...
| `erd` | `title` `description` `dir` · `entities[]`: `name*` `kind` `schema` `note` `columns[]`: `name*` `type` `pk`(bool) `fk`(bool) `unique`(bool) `nullable`(bool) `default` `index`(bool) `enum[]` `ref` `note` · `indexes[]`: `columns[]*` `unique` `name` · `relations[]`: `from*` `to*` `label` `card` `identifying`(bool) `fromCol` `toCol` · `groups[]`: `name*` `entities[]` · `enums[]`: `name*` `values[]*` | card: "1:1" · "1:N" · "N:1" · "N:M" · "0..1" · "0..N" (quote!); kind: table·view·enum·external; dir: LR·TB |
...
| `block` | `preset` `systemLabel` `dir` · `layers[]`: `label*` `color` · `groups[]`: `id` `parent` `col*`(n) `row*`(n) `cols`(n) `rows`(n) `label*` `color` · `nodes[]`: `id*` `name*` (`col`(n)+`row`(n) **or** `layer`(n)) `w`(n) `kind` `tech` `replicas`(n) · `edges[]`: `from*` `to*` `label` `kind` | preset: infra · event · ddd · network · k8s (framing + entry kind only — same YAML) — group `parent`: another group's `id` (child cells inside the parent's, else `W_GROUP_NESTING`) — node `replicas`: integer ≥ 1 (≥ 2 draws the stack + `×N`) — node kind: free string (client · service · microservice · db · cache · queue · gateway · cdn · external · ingress · deployment · pod · configmap · secret · cronjob · node · …) — edge kind: solid · dashed · forbidden · error |
...
| `eventcontract` | `name*` `version` `channel` `summary` `producers[]` `consumers[]` `delivery` `ordering` `key` `retention` `example` `note` · `schema[]`: `name*` `type*` `required`(bool) `desc` `example` · `headers[]`: same shape · `errors[]`: `name*` `when` | delivery: at-least-once · at-most-once · exactly-once — ordering: none · per-key · global. `key` names the partition key; the payload row with that name is marked `#` and takes the accent, optional fields are marked `?`. String items: `name type [required] — desc` for `schema`/`headers`, `Name — when` for `errors` |
| `saga` | `mode` `coordinator` `failAt` · `steps*[]` (1+): `id*` `name*` `service*` `action` `compensate` `status` | mode: orchestration (a coordinator band on top, fanning out to every step) · choreography (default, no band) — status: ok · failed · skipped · compensated. `failAt` is a step `id`; it derives the statuses (before it compensated, at it failed, after it skipped) unless a step sets its own. String steps: `id: Name · service · compensate` (two parts: name · service; four: name · service · action · compensate) |
...
| `spans` | `unit` · `spans*[]`: `id*` `name*` `service*` `start*`(n) `duration*`(n) `parent` `kind` `error` `attrs` `note` | unit: ms (default) · s · us. kind: server · client · db · queue · cache · internal. `parent` must name another span's `id`; ids are unique. `attrs` is a map of string/number values (listed under the drawing with `note`). Terse item: `service/id: name · start · duration [· parent]`. Lanes are services in first-appearance order; the critical path (root → longest child at each hop) takes the accent. Density warns past 40 spans |
| `rollout` | `strategy` · `stages*[]`: `name*` `traffic`(n, 0–100) `duration` `gate` `status` `note` · `rollback` | strategy: canary · blue-green · rolling · feature-flag. status: done · current · next · blocked. `gate` is what must pass before the NEXT stage starts (drawn on the connector). Terse stage (quote it when it starts with a bracket): `"[status] traffic% · name · duration — gate"`; every part but the name is optional |
