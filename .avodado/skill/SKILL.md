...
#### `gallery` — a responsive grid of code / note cards
```gallery
title: Bug gallery
description: A grid of code snippets — great for a "spot the bug" set or a comparison.
items:
...
```
It's a real grid (**2 columns by default**; set `cols` to 3–4). A cell is one of:
a `code` snippet (with a `title`/`lang` header + optional `caption`), a plain
note (`title` + `caption`), or a **nested `block`** — `block: { type: <block>, …data }` —
so a cell can hold a whole diagram. That makes `gallery` the way to **compare
architectures side by side**: `cols: 3` with three `c4`/`block` cells. The nested
block is validated against its own schema. Reach for `gallery` (not a multi-block
`code`) when you want cards/diagrams in a grid rather than stacked.
...
> for code; `block`/`flow`/`state`/`sequence` for system & AI). `avo design --all`
> (or `--system` / `--ai` / `--code`) renders the gallery. When the user names a
