...
description: >-
  ...
  wireframe · endpoint · pullquote · layers · matrix · anatomy · composition).
...
4. **Pick 2-5 blocks that carry the structure.** Don't try to use all 46 in one doc.
...
types, and don't reach for all 46. Start from the doc's job and pick a stack:
...
- The info-string is exactly one of the **46 block types** listed below — never
  invent new ones.
...
| `composition` | `title` `description` `result` · `gates[]`: `label*` `desc` | renders `gate₁ ∩ gate₂ ∩ … = result` |
