...
4. **Pick 2-5 blocks that carry the structure.** Don't try to use all 49 in one doc.
...
types, and don't reach for all 49. Start from the doc's job and pick a stack:
...
| **Presentation / deck** | "slides", "deck", "present this", "pitch" | `meta` (cover) → **one slide per `---`**, each a `#` heading + one strong block (`drivers`/`stats`/`pyramid`/`quadrant`/`timeline`/a diagram). See *Slide decks* below. |
...
Any document renders as a deck with `avo slides`. **You control pagination with
`---`** (a Markdown thematic break on its own line): everything between two `---`
is one slide, and the slide can hold several blocks plus prose. The first `#`/`##`
heading inside a slide is its title (shown at the top, not repeated in the body).
...
- Keep each slide to **one idea** — a heading plus one strong visual (a diagram,
  `drivers`, `stats`, `pyramid`, `quadrant`, `timeline`) reads better than dense prose.
- The `meta` block is the cover slide. If a doc has **no** `---`, each block becomes
  its own slide (legacy behavior).
...
- The info-string is exactly one of the **49 block types** listed below — never
  invent new ones.
