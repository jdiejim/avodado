...
const VALID_THEMES: ReadonlyArray<ThemeName> = ['textbook', 'minimal', 'teal', 'plum', 'slate'];

function readTheme(): ThemeName {
  const cfg = vscode.workspace.getConfiguration('avodado');
  const t = cfg.get<string>('theme', 'textbook');
  return (VALID_THEMES as readonly string[]).includes(t) ? (t as ThemeName) : 'textbook';
}
