---
description: Authors, edits, and fixes Avodado documentation (Markdown + typed YAML blocks) under docs/.
...
typed, fenced YAML blocks (42 block types). The files on disk are the single source
of truth — there is no separate database or UI to update.

Follow the Avodado authoring skill (`.avodado/skill/SKILL.md`). It is the authority
on the block grammar, every block's fields, and the `doc#id` reference scheme.
