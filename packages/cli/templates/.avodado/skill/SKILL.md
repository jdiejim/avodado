...
4. **Pick 2-5 blocks that carry the structure.** Don't try to use all 42 in one doc.
...
types, and don't reach for all 42. Start from the doc's job and pick a stack:
...
- The info-string is exactly one of the **42 block types** listed below — never
  invent new ones.
