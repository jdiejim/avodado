...
items:
  ...
  - { title: Typed blocks, body: "53 block types with strict schemas.", tag: "HOW", icon: grid, accent: green }
...
blocks:
  - title: The shape of every block
    ...
    code: |
      ```stats
      stats:
        - { value: "53", label: Block types, trend: flat }
      ```
```
...
Don't hand-draw a known pattern — `avo design` ships 80 (system design · AI/agents
· GoF code), each a ready card **plus a fitting diagram**.
