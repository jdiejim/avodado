...
describe.skipIf(skipIfNotBuilt)('avo serve (built bin)', () => {
  it('serves the site from memory, injects live reload, and exits cleanly on SIGINT', async () => {
    ...
    try {
      ...
      expect(pageHtml).toContain('class="view-toggle"');
      expect(pageHtml).toContain('<a href="getting-started.slides.html">Slides</a>');

      // `/<slug>.slides.html` → the deck view: deck markup + back-link pill +
      // the same live-reload script.
      const deckRes = await fetch(`http://127.0.0.1:${port}/getting-started.slides.html`);
      expect(deckRes.status).toBe(200);
      const deckHtml = await deckRes.text();
      expect(deckHtml).toContain('class="deck-nav"');
      expect(deckHtml).toContain('class="deck-doc-link" href="getting-started.html"');
      expect(deckHtml).toContain(`new EventSource('/__events').onmessage=()=>location.reload()`);
