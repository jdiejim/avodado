  ...
  it('scaffolds base files + all tool config (instructions, skill, agents) by default', async () => {
    ...
    try {
      ...
      for (const f of [
        ...
        '.github/skills/avodado-docs/reference/intake.md',
        '.github/skills/avodado-docs/reference/blocks/INDEX.md',
      ]) {
        expect(result.created).toContain(f);
      }
      // Copilot Agent Skills use the same SKILL.md folder format — the hub +
      // reference/ spokes install verbatim (no stitched prompt file anymore).
      expect(result.created).not.toContain('.github/prompts/avodado-docs.prompt.md');
      const hub = await readFile(join(root, '.github/skills/avodado-docs/SKILL.md'), 'utf8');
      expect(hub).toContain('live beside this file — read them on demand');
      const intake = await readFile(join(root, '.github/skills/avodado-docs/reference/intake.md'), 'utf8');
      expect(intake).toContain('Reviewing an existing doc');
