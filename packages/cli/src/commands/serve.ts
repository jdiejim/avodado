...
export interface ServeOptions {
  ...
  /** Serve the rich index page. On by default; `--no-rich-index` (or config
...
export async function runServe(opts: ServeOptions): Promise<void> {
  ...
  const server = createServer((req, res) => {
    ...
    // Diagnostics banner rides on every page while problems exist — including
    // deck pages: it overlays the deck's bottom nav bar, but errors must stay
    // visible in either view and keyboard navigation still works.
  ...
  const onFsEvent = (): void => {
    ...
    debounce = setTimeout(() => {
      void (async () => {
        await rebuild();
        docsWatcher.resync(); // fallback mode: pick up new directories
        broadcastReload();
      })();
    }, DEBOUNCE_MS);
  };
