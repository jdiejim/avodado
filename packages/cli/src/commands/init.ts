...
export function themeFileContents(theme: string, custom: boolean): string {
...
const isSkillDest = (dest: string): boolean =>
  dest.endsWith('SKILL.md') || dest.endsWith('.prompt.md');
...
export async function installTool(opts: {
  ...
  const files = [{ src: CANONICAL_SKILL, dest: CANONICAL_SKILL }, ...TOOL_FILES[opts.tool]];
