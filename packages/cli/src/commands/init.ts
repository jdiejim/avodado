 ...
 * native skill location, so all tools share one identical skill. Copilot's
 * prompt is a single file with no folder beside it, so it receives the
 * *stitched* skill (hub + references inlined — see {@link stitchSkill}).
 * Agents are generated only for the tools that actually have an agent format
 * (Claude, Copilot).
 */
