...
export const BUILTIN_PROMPTS: readonly PromptDef[] = [
  ...
  },
  {
    ...
    text: `Write an Avodado slide deck about <TOPIC> (renders with \`avo slides\`).\nSeparate slides with \`---\` on its own line; each slide is a \`#\`/\`##\` heading (the slide title) plus one focused block — open with a \`meta\` cover, then \`drivers\` / \`stats\` / \`pyramid\` / \`quadrant\` / \`timeline\` and diagrams that read at a glance. One strong visual per slide, not dense prose.\n${tail}`,
  },
];
