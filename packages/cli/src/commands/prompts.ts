...
export const BUILTIN_PROMPTS: readonly PromptDef[] = [
  ...
  },
  {
    ...
    text: `Write an Avodado document about <TOPIC> designed to present as slides (one section per slide via \`avo slides\`).\nKeep each \`##\` section to a single focused block with a clear title: open with a \`meta\` cover, then \`drivers\` / \`stats\` / \`pyramid\` / \`quadrant\` / \`timeline\` and diagrams that read at a glance. Prefer one strong visual per slide over dense prose.\n${tail}`,
  },
];
