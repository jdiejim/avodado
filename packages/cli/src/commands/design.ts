...
/** A pattern category. The three GoF families, the architecture classics, plus system design. */
...
/** True for any code-pattern category (the GoF families + the architecture classics). */
const isCode = (c: DesignCategory): boolean =>
...
const SYSTEM: readonly DesignPattern[] = [
  ...
  },
  {
    ...
    category: 'System design',
    summary: 'Retry transient failures with exponential, jittered delays.',
    intent: 'Reattempt a failed call with exponentially growing, jittered delays until it succeeds or the budget runs out.',
    forces: ['Most remote failures are transient', 'Naive retries amplify an outage', 'Callers cannot wait forever'],
    participants: [
      { name: 'Caller', role: 'wraps the call in a retry policy' },
      { name: 'Backoff policy', role: 'exponential delay + jitter per attempt' },
      { name: 'Retry budget', role: 'caps attempts, then surfaces the error' },
    ],
    pros: ['Rides out transient blips', 'Jitter breaks up synchronized retry storms'],
    cons: ['Adds tail latency on failure', 'Retrying non-idempotent calls duplicates work'],
  },
  {
    slug: 'bulkhead',
    name: 'Bulkhead',
    category: 'System design',
    summary: 'Partition resources into isolated pools so one failure cannot sink the ship.',
    intent: 'Give each dependency its own bounded pool so exhaustion in one compartment cannot starve the others.',
    forces: ['One slow dependency can hog every thread', 'Blast radius must be contained', 'Total capacity is finite'],
    participants: [
      { name: 'Pools', role: 'bounded threads/connections per dependency' },
      { name: 'Router', role: 'sends each call to its own pool' },
      { name: 'Rejection policy', role: 'fails fast when a pool is full' },
    ],
    pros: ['Failure stays in one compartment', 'Predictable capacity per dependency'],
    cons: ['Idle capacity in quiet pools', 'Pool sizing is another knob to tune'],
  },
  {
    slug: 'timeout',
    name: 'Timeout',
    category: 'System design',
    summary: 'Bound every remote call so a slow dependency cannot hold you hostage.',
    intent: 'Abort a call that exceeds its deadline and surface a fast, explicit failure instead of hanging.',
    forces: ['Remote calls can hang indefinitely', 'Held threads and sockets starve the caller', 'Slow is often worse than down'],
    participants: [
      { name: 'Caller', role: 'sets a deadline per call' },
      { name: 'Timer', role: 'cancels the attempt when the deadline passes' },
      { name: 'Fallback', role: 'error, cached value, or retry on expiry' },
    ],
    pros: ['Bounds worst-case latency', 'Frees resources for healthy work'],
    cons: ['A good value is workload-specific', 'Aborted work may still complete server-side'],
  },
  {
    slug: 'cache-aside',
    name: 'Cache-aside',
    category: 'System design',
    summary: 'The app checks the cache; on a miss it loads from the store and populates.',
    intent: 'Keep the cache lazy — read it first, and on a miss fetch from the store and write the entry back.',
    forces: ['Hot keys dominate reads', 'The cache must not become a second source of truth', 'Cache loss must degrade, not break'],
    participants: [
      { name: 'Application', role: 'owns the miss logic: read, load, populate' },
      { name: 'Cache', role: 'holds entries with a TTL' },
      { name: 'Store', role: 'source of truth on a miss' },
    ],
    pros: ['Only requested data gets cached', 'Cache failure degrades to slower reads'],
    cons: ['First hit pays a double read', 'Stale window until TTL or invalidation'],
  },
  {
    slug: 'throttling',
    name: 'Throttling',
    category: 'System design',
    summary: 'Smooth bursts by spending tokens that refill at a fixed rate.',
    intent: 'Admit work at a sustainable rate — each request consumes a token and the bucket refills steadily.',
    forces: ['Bursts exceed steady capacity', 'Shedding early beats melting down', 'Some burst tolerance is still wanted'],
    participants: [
      { name: 'Token bucket', role: 'capacity = burst allowance, refill = sustained rate' },
      { name: 'Gate', role: 'admits a request only when a token is available' },
      { name: 'Overflow policy', role: 'queues, delays, or rejects the excess' },
    ],
    pros: ['Absorbs bursts up to the bucket size', 'Downstreams see a bounded rate'],
    cons: ['Rejected callers need a retry story', 'A shared bucket needs a shared store'],
  },
  {
    slug: 'actor-model',
    name: 'Actor model',
    category: 'System design',
    summary: 'Concurrency via isolated actors that only talk through mailboxes.',
    intent: 'Confine state to actors that process one mailbox message at a time and communicate only by sending more messages.',
    forces: ['Shared-memory locking is error-prone', 'State wants single-threaded safety', 'Millions of small concurrent units'],
    participants: [
      { name: 'Actor', role: 'private state + behavior, one message at a time' },
      { name: 'Mailbox', role: 'queue of pending messages per actor' },
      { name: 'Supervisor', role: 'restarts failed children per a strategy' },
    ],
    pros: ['No locks — state is confined per actor', 'Supervision gives clean failure recovery'],
    cons: ['Indirect flow is harder to trace', 'Mailbox overflow needs backpressure'],
  },
  {
    slug: 'producer-consumer',
    name: 'Producer–consumer',
    category: 'System design',
    summary: 'Decouple work creation from work processing with a bounded buffer.',
    intent: 'Let producers enqueue into a bounded buffer that consumers drain at their own pace.',
    forces: ['Producers and consumers run at different speeds', 'Bursts need smoothing', 'An unbounded buffer just hides overload'],
    participants: [
      { name: 'Producers', role: 'push work; block or shed when the buffer is full' },
      { name: 'Bounded buffer', role: 'queue with a fixed capacity' },
      { name: 'Consumers', role: 'pull and process at their own rate' },
    ],
    pros: ['Decouples rates and smooths bursts', 'Either side scales independently'],
    cons: ['The bound forces a full-buffer policy', 'End-to-end latency hides in the queue'],
  },
  {
    slug: 'thread-pool',
    name: 'Thread pool',
    category: 'System design',
    summary: 'Reuse a fixed set of workers instead of spawning a thread per task.',
    intent: 'Queue tasks to a fixed pool of long-lived workers so concurrency stays bounded and spawn cost is paid once.',
    forces: ['Thread creation is expensive', 'Unbounded concurrency thrashes the machine', 'Task arrival is spiky'],
    participants: [
      { name: 'Task queue', role: 'holds submitted work' },
      { name: 'Workers', role: 'fixed threads looping on the queue' },
      { name: 'Pool manager', role: 'sizes the pool, handles rejection' },
    ],
    pros: ['Bounded, predictable concurrency', 'Amortizes thread start-up cost'],
    cons: ['Blocking tasks can starve the pool', 'Right size is workload-dependent'],
  },
  {
    slug: 'competing-consumers',
    name: 'Competing consumers',
    category: 'System design',
    summary: 'Identical consumers race on one queue; each message goes to exactly one.',
    intent: 'Scale queue processing horizontally by letting interchangeable consumers compete for messages.',
    forces: ['One consumer cannot keep up', 'Work items are independent', 'Strict ordering is not required'],
    participants: [
      { name: 'Queue', role: 'delivers each message to a single consumer' },
      { name: 'Consumers', role: 'identical workers pulling in parallel' },
      { name: 'Visibility timeout', role: 'returns a message when its consumer dies' },
    ],
    pros: ['Near-linear throughput scaling', 'A dead consumer just loses the race'],
    cons: ['Order is lost across consumers', 'Redelivery demands idempotent handlers'],
  },
  {
    slug: 'splitter-aggregator',
    name: 'Splitter–aggregator',
    category: 'System design',
    summary: 'Split a composite message into parts, process in parallel, recombine.',
    intent: 'Break one composite request into independent parts, fan them out, and aggregate the results into a single reply.',
    forces: ['A request bundles independent parts', 'Parallelism cuts latency', 'The reply needs all (or enough) parts back'],
    participants: [
      { name: 'Splitter', role: 'breaks the message into correlated parts' },
      { name: 'Branches', role: 'process the parts concurrently' },
      { name: 'Aggregator', role: 'matches parts by correlation id, emits the whole' },
    ],
    pros: ['Latency of the slowest part, not the sum', 'Branches scale independently'],
    cons: ['Aggregator must handle partial failure', 'Correlation state to keep'],
  },
];
...
const CODE: readonly DesignPattern[] = [
  ...
  },
  {
    ...
    name: 'Interpreter',
    category: 'Behavioral',
    summary: 'Model a grammar as a class per rule and evaluate sentences over a context.',
    intent: 'Define a representation for a language’s grammar along with an interpreter that evaluates sentences in it.',
    forces: ['A small recurring language (filters, formulas, rules)', 'The grammar is simple and stable', 'Ad-hoc parsing keeps spreading'],
    participants: [
      { name: 'AbstractExpression', role: 'declares interpret(context)' },
      { name: 'TerminalExpression', role: 'interprets a leaf symbol' },
      { name: 'NonterminalExpression', role: 'composes sub-expressions per grammar rule' },
      { name: 'Context', role: 'global state, e.g. variable bindings' },
    ],
    pros: ['Each grammar rule is an explicit class', 'New rules slot in without touching old ones'],
    cons: ['Class-per-rule explodes on big grammars', 'A parser generator often fits better'],
  },
  // Architecture
  {
    slug: 'mvc',
    ...
    summary: 'Split a UI into model (state), view (render), and controller (input).',
    intent: 'Separate application state, its presentation, and input handling so each can change independently.',
    forces: ['UI and domain logic tangle fast', 'One model feeds many views', 'Input handling is its own concern'],
    participants: [
      { name: 'Model', role: 'state + domain rules, UI-agnostic' },
      { name: 'View', role: 'renders the model, observes changes' },
      { name: 'Controller', role: 'translates input into model updates' },
    ],
    pros: ['Model is testable with no UI attached', 'Many views over one model'],
    cons: ['Boundaries blur in practice (fat controllers)', 'Indirection overhead for tiny UIs'],
  },
  {
    slug: 'mvvm',
    name: 'Model–View–ViewModel',
    category: 'Architecture',
    summary: 'A bindable view-model mirrors the view, so the view stays declarative.',
    intent: 'Expose view state and commands on a view-model the view data-binds to, leaving the view logic-free.',
    forces: ['View logic is hard to unit-test', 'A data-binding engine is available', 'Designers and developers work in parallel'],
    participants: [
      { name: 'Model', role: 'domain data + rules' },
      { name: 'ViewModel', role: 'view state + commands, holds no view reference' },
      { name: 'View', role: 'declarative markup bound to the view-model' },
    ],
    pros: ['View-model tests run without any UI', 'Two-way binding removes glue code'],
    cons: ['Binding failures are silent and hard to debug', 'View-models bloat over time'],
  },
  {
    slug: 'dependency-injection',
    name: 'Dependency injection',
    category: 'Architecture',
    summary: 'Hand a class its dependencies instead of letting it construct them.',
    intent: 'Invert construction — dependencies arrive through the constructor, and wiring lives in one composition root.',
    forces: ['new inside business code welds classes together', 'Tests need to substitute doubles', 'Object graphs run deep'],
    participants: [
      { name: 'Consumer', role: 'declares what it needs via its constructor' },
      { name: 'Abstraction', role: 'the interface the consumer depends on' },
      { name: 'Composition root', role: 'builds the graph and injects implementations' },
    ],
    pros: ['Swap real for fake in tests', 'All wiring is visible in one place'],
    cons: ['Container magic obscures construction', 'Ceremony for shallow graphs'],
  },
  {
    slug: 'unit-of-work',
    name: 'Unit of Work',
    category: 'Architecture',
    summary: 'Track everything a business transaction touches, then commit it atomically.',
    intent: 'Record the objects a transaction creates, changes, and removes, then flush all writes in one coordinated commit.',
    forces: ['Chatty per-object saves hammer the database', 'A business action must be all-or-nothing', 'Write order matters'],
    participants: [
      { name: 'UnitOfWork', role: 'registers new / dirty / removed objects' },
      { name: 'Identity map', role: 'one in-memory instance per loaded row' },
      { name: 'Commit', role: 'orders and flushes all changes in one transaction' },
    ],
    pros: ['One atomic flush per transaction', 'Batches and dedupes writes'],
    cons: ['Long units hold locks and memory', 'Deferred flushes surprise debuggers'],
  },
  {
    slug: 'active-record',
    name: 'Active Record',
    category: 'Architecture',
    summary: 'A row-shaped object that knows how to load and save itself.',
    intent: 'Wrap one table row in an object that carries both the data and its own persistence methods.',
    forces: ['CRUD apps want minimal ceremony', 'The domain shape mirrors the tables', 'One class per table is easy to navigate'],
    participants: [
      { name: 'ActiveRecord base', role: 'find / save / delete against the table' },
      { name: 'Record class', role: 'columns as fields + domain methods' },
      { name: 'Table', role: 'one row per instance' },
    ],
    pros: ['Fast to write, obvious to read', 'Perfect fit for schema-shaped domains'],
    cons: ['Domain is welded to the schema', 'Hard to test without a database'],
  },
  {
    slug: 'data-mapper',
    name: 'Data Mapper',
    category: 'Architecture',
    summary: 'A mapper layer moves data between pure domain objects and the database.',
    intent: 'Keep domain objects persistence-ignorant by putting all load/save translation in a separate mapper.',
    forces: ['The domain model diverges from the schema', 'Domain logic must be testable in isolation', 'Mapping code has to live somewhere'],
    participants: [
      { name: 'Domain object', role: 'pure behavior, no SQL' },
      { name: 'Mapper', role: 'translates objects ↔ rows in both directions' },
      { name: 'Database', role: 'schema free to differ from the model' },
    ],
    pros: ['Domain tests need no database', 'Schema and model evolve independently'],
    cons: ['A whole mapping layer to maintain', 'Overkill for plain CRUD'],
  },
  {
    slug: 'event-bus',
    name: 'Event bus',
    category: 'Architecture',
    summary: 'In-process pub/sub — modules emit events, subscribers react, nobody links.',
    intent: 'Decouple modules inside one process by publishing typed events on a shared bus instead of calling each other.',
    forces: ['Direct calls weave a dependency web', 'Side-effects keep accreting onto core actions', 'Emitters should not know their audience'],
    participants: [
      { name: 'Publisher', role: 'emits an event, fire-and-forget' },
      { name: 'Bus', role: 'routes each event to its subscribers' },
      { name: 'Subscribers', role: 'handle the events they registered for' },
    ],
    pros: ['New reactions without touching the emitter', 'Modules stay unlinked'],
    cons: ['Control flow turns invisible', 'Ordering + error handling get fuzzy'],
  },
  {
    slug: 'specification',
    name: 'Specification',
    category: 'Architecture',
    summary: 'Business rules as composable predicate objects — and, or, not.',
    intent: 'Encapsulate a selection rule in an object with isSatisfiedBy(), composable through boolean combinators.',
    forces: ['The same rule appears in queries, validation, and UI', 'Rules combine in ad-hoc ways', 'if-soup buries the business language'],
    participants: [
      { name: 'Specification', role: 'declares isSatisfiedBy(candidate)' },
      { name: 'Concrete specs', role: 'one named business rule each' },
      { name: 'Combinators', role: 'and / or / not build composite rules' },
    ],
    pros: ['Rules are named, tested, reusable', 'Composites read like the business language'],
    cons: ['Class-per-rule ceremony', 'Translating specs into SQL is nontrivial'],
  },
  {
    slug: 'null-object',
    name: 'Null Object',
    category: 'Architecture',
    summary: 'Return a do-nothing implementation instead of null.',
    intent: 'Replace null checks with a neutral object that implements the interface by safely doing nothing.',
    forces: ['null checks litter every call site', 'A missing collaborator has a sensible default', 'One forgotten check crashes'],
    participants: [
      { name: 'Abstraction', role: 'the interface callers depend on' },
      { name: 'RealObject', role: 'the working implementation' },
      { name: 'NullObject', role: 'safe no-op defaults for every method' },
    ],
    pros: ['Call sites lose their null checks', 'Absence gets defined behavior'],
    cons: ['Silently doing nothing can mask bugs', 'One null class per interface'],
  },
];
...
const AI: readonly DesignPattern[] = [
  ...
  },
  {
    ...
    category: 'AI / agents',
    summary: 'Draft a multi-step plan first, execute it, replan when reality disagrees.',
    intent: 'Separate planning from acting — a planner writes the step list, an executor works through it, and drift triggers a replan.',
    forces: ['Long tasks wander without a plan', 'Re-deciding at every step burns tokens', 'The world changes mid-task'],
    participants: [
      { name: 'Planner', role: 'turns the goal into ordered steps' },
      { name: 'Executor', role: 'carries out one step at a time' },
      { name: 'Replanner', role: 'revises the remaining steps on surprises' },
    ],
    pros: ['Coherent multi-step behavior', 'The plan is auditable before any action'],
    cons: ['Plans go stale without a replan trigger', 'Two roles to prompt and tune'],
  },
  {
    slug: 'human-in-the-loop',
    name: 'Human-in-the-loop',
    category: 'AI / agents',
    summary: 'Pause the agent at consequential actions until a human approves.',
    intent: 'Gate irreversible or risky agent actions behind an explicit human approval step.',
    forces: ['Some actions cannot be undone', 'Full autonomy is not yet trusted', 'Human attention is slow and finite'],
    participants: [
      { name: 'Agent', role: 'proposes the action + its rationale' },
      { name: 'Approval gate', role: 'suspends the run until a decision arrives' },
      { name: 'Reviewer', role: 'approves, edits, or rejects' },
    ],
    pros: ['Catches bad actions before they land', 'Approvals double as training data'],
    cons: ['Human latency caps throughput', 'Rubber-stamping erodes the safeguard'],
  },
  {
    slug: 'agentic-rag',
    name: 'Agentic RAG',
    category: 'AI / agents',
    summary: 'The agent decides when, what, and where to retrieve — and loops until satisfied.',
    intent: 'Make retrieval a tool the agent calls deliberately, grading results and re-querying instead of one fixed lookup.',
    forces: ['One-shot retrieval misses multi-hop questions', 'Not every query needs retrieval', 'Bad chunks deserve a second attempt'],
    participants: [
      { name: 'Agent', role: 'decides to search, judges the evidence' },
      { name: 'Search tool', role: 'vector / keyword / SQL retrieval on demand' },
      { name: 'Grader', role: 'checks the chunks actually answer the question' },
    ],
    pros: ['Handles multi-hop and vague questions', 'Skips retrieval when it is not needed'],
    cons: ['More calls, more latency', 'The loop can chase its tail'],
  },
  {
    slug: 'swarm-handoff',
    name: 'Swarm handoff',
    category: 'AI / agents',
    summary: 'Peer agents pass the live conversation to whichever specialist fits next.',
    intent: 'Let each agent transfer the conversation — context included — to a better-suited peer instead of routing through a boss.',
    forces: ['A dialog crosses specialist domains mid-stream', 'A central orchestrator adds a hop', 'Context must survive the transfer'],
    participants: [
      { name: 'Agents', role: 'peers, each with its own tools + instructions' },
      { name: 'Handoff tool', role: 'transfers control to a named peer' },
      { name: 'Shared context', role: 'history + variables that travel along' },
    ],
    pros: ['The right specialist at each moment', 'No central bottleneck'],
    cons: ['Ping-pong handoffs can loop', 'A hop chain is hard to debug'],
  },
  {
    slug: 'chain-of-thought',
    name: 'Chain-of-thought',
    category: 'AI / agents',
    summary: 'Ask the model to reason step by step before it commits to an answer.',
    intent: 'Elicit intermediate reasoning ahead of the final answer so hard problems are worked, not guessed.',
    forces: ['Direct answers fail on multi-step problems', 'Reasoning tokens cost money', 'The trace helps review even when wrong'],
    participants: [
      { name: 'Prompt', role: 'instructs step-by-step work, or shows worked examples' },
      { name: 'Reasoning trace', role: 'the intermediate steps, kept separable' },
      { name: 'Final answer', role: 'extracted after the reasoning ends' },
    ],
    pros: ['Big gains on math, logic, planning', 'The trace shows where reasoning broke'],
    cons: ['More tokens + latency', 'A fluent trace can still rationalize a wrong answer'],
  },
  {
    slug: 'context-compaction',
    name: 'Context compaction',
    category: 'AI / agents',
    summary: 'When the window nears full, summarize the old turns and keep going.',
    intent: 'Replace stale conversation history with a dense summary so long sessions never overflow the window.',
    forces: ['Sessions outlive the context window', 'Early decisions must not be forgotten', 'Every summary loses some detail'],
    participants: [
      { name: 'Monitor', role: 'watches token usage against a threshold' },
      { name: 'Summarizer', role: 'compresses the old turns into a brief' },
      { name: 'Window', role: 'summary + recent turns + reclaimed budget' },
    ],
    pros: ['Sessions can run indefinitely', 'Recent detail survives intact'],
    cons: ['Summarization drops nuance', 'A bad summary poisons every later turn'],
  },
];

/** The full library: system design, AI/agent patterns, then the code patterns (GoF + architecture). */
...
/**
 * Hand-authored structure diagram per pattern, keyed by slug. GoF code patterns
 * use a `uml` class diagram (proper inheritance / implementation / aggregation
 * markers); architecture classics use `uml`, `belogic`, or a `sequence`; system
 * and AI patterns use whichever shape fits — a `block`/`event` topology, a
 * `flow` decision, a `state` machine, a `sequence`, a `swimlane`, a `prompt`,
 * or a `context` window. Each is tailored, so no two look alike.
 */
...
description: The service checks the cache first and only hits the repository on a miss.
nodes:
  - { id: ctl, col: 1, row: 1, kind: controller, name: Handler, note: GET /item }
  - { id: svc, col: 2, row: 1, kind: service, name: ItemService, note: read-through }
  - { id: cache, col: 2, row: 2, kind: cache, name: Redis, note: hot items }
  - { id: repo, col: 3, row: 1, kind: repository, name: ItemRepo, note: on miss }
  - { id: db, col: 3, row: 2, kind: db, name: Postgres, note: source of truth }
edges:
  - { from: ctl, to: svc, kind: uses }
  - { from: svc, to: cache, kind: reads }
...
description: Edge PoPs cache near users; a miss falls back to the origin inside the cloud account.
groups:
  - { id: cloud, label: Cloud account, col: 2, row: 1, cols: 1, rows: 2, color: "#374151" }
nodes:
  - { id: u, col: 1, row: 1, kind: client, name: Users, tech: global }
  - { id: edge, col: 1, row: 2, kind: cdn, name: Edge PoPs, tech: CDN }
  - { id: origin, col: 2, row: 1, kind: compute, name: Origin, tech: containers }
  - { id: bucket, col: 2, row: 2, kind: bucket, name: Assets, tech: object store }
edges:
  - { from: u, to: edge, label: nearest PoP }
  - { from: edge, to: origin, label: miss, kind: dashed }
  - { from: origin, to: bucket, label: reads }`,
  ),
  sharding: fence(
    'block',
...
description: Each pod runs the app next to a sidecar proxy that owns networking; they share localhost.
clusters:
  - { id: pod, label: Pod, kind: namespace }
services:
  - { id: app, cluster: pod, label: App, kind: service, tech: Go, replicas: 3 }
  - { id: side, cluster: pod, label: Sidecar, kind: service, tech: Envoy, replicas: 3 }
  - { id: cfg, cluster: pod, label: Config agent, kind: service, tech: agent, replicas: 3 }
edges:
  - { from: app, to: side, label: localhost }
  - { from: cfg, to: side, kind: dashed }`,
  ),
...
description: A connector tails the DB log and publishes change events; consumers each subscribe.
systemLabel: cdc
layers:
  - { label: Source }
  - { label: Capture }
  - { label: Consumers }
nodes:
  - { id: db, layer: 0, kind: db, name: Database, tech: Postgres }
  - { id: conn, layer: 1, kind: service, name: CDC connector, tech: Debezium }
  - { id: stream, layer: 1, kind: topic, name: Change stream, tech: Kafka }
  - { id: search, layer: 2, kind: service, name: Search }
  - { id: cache, layer: 2, kind: cache, name: Cache }
  - { id: dw, layer: 2, kind: store, name: Warehouse }
edges:
...
description: Producers publish to a broker; independent consumers each react in their own band.
systemLabel: events
layers:
  - { label: Producers }
  - { label: Backbone }
  - { label: Consumers }
nodes:
  - { id: o, layer: 0, kind: service, name: Orders, tech: Go }
  - { id: p, layer: 0, kind: service, name: Payments, tech: Java }
  - { id: bus, layer: 1, kind: topic, name: Event broker, tech: Kafka }
  - { id: e, layer: 2, kind: service, name: Email, tech: worker }
  - { id: a, layer: 2, kind: service, name: Analytics, tech: Spark }
  - { id: s, layer: 2, kind: service, name: Search index, tech: Elastic }
edges:
  - { from: o, to: bus }
  - { from: p, to: bus }
  - { from: bus, to: e }
  ...
  microservices: fence(
    'c4',
    `title: Microservices
description: A gateway fronts independent services; each owns its data and payments is external.
level: container
boundary: { label: Shop platform }
nodes:
  - { id: u, col: 1, row: 2, kind: person, name: Customer, desc: Places orders from web or mobile. }
  - { id: gw, col: 2, row: 2, kind: container, family: service, name: API gateway, tech: Envoy, desc: Routes + authenticates. }
  ...
  - { id: db, col: 4, row: 1, kind: store, name: Orders DB, tech: Postgres }
  - { id: stripe, col: 4, row: 2, kind: external, name: Stripe, desc: Card authorisation. }
edges:
  - { from: u, to: gw, label: HTTPS }
  - { from: gw, to: ord, label: /orders }
  - { from: gw, to: pay, label: /pay }
  - { from: gw, to: inv, label: /stock }
  - { from: ord, to: db, label: writes }
  - { from: pay, to: stripe, label: authorises }`,
  ),
...
description: A control plane configures per-service sidecars; app traffic flows sidecar-to-sidecar over mTLS.
level: container
boundary: { label: Mesh }
nodes:
  - { id: cp, col: 1, row: 1, kind: container, family: controller, name: Control plane, tech: istiod, desc: Pushes policy + config. }
  - { id: a, col: 2, row: 1, kind: container, family: service, name: Orders, tech: Go }
  - { id: pa, col: 3, row: 1, kind: container, family: client, name: Sidecar, tech: Envoy }
  - { id: b, col: 2, row: 2, kind: container, family: service, name: Payments, tech: Java }
  - { id: pb, col: 3, row: 2, kind: container, family: client, name: Sidecar, tech: Envoy }
edges:
  - { from: a, to: pa, label: localhost }
  - { from: b, to: pb, label: localhost }
...
edges:
  ...
  'retry-backoff': fence(
    'flow',
    `title: Retry until the budget runs out
nodes:
  - { id: call, col: 1, row: 1, kind: start, label: Call payment API }
  - { id: ok, col: 2, row: 1, kind: decision, label: "Succeeded?" }
  - { id: done, col: 3, row: 1, kind: end, label: Return result }
  - { id: left, col: 2, row: 2, kind: decision, label: "Attempts left?" }
  - { id: wait, col: 1, row: 2, kind: process, label: "Sleep 2^n + jitter" }
  - { id: give, col: 3, row: 2, kind: end, label: Surface error }
edges:
  - { from: call, to: ok }
  - { from: ok, to: done, label: "yes" }
  - { from: ok, to: left, label: "no" }
  - { from: left, to: wait, label: "yes" }
  - { from: wait, to: call, label: retry }
  - { from: left, to: give, label: "no", kind: error }`,
  ),
  bulkhead: fence(
    'block',
    `title: One compartment floods, the rest sail
groups:
  - { id: pools, col: 2, row: 1, cols: 1, rows: 3, label: Isolated pools, color: "#6b7280" }
nodes:
  - { id: api, col: 1, row: 2, kind: gateway, name: Checkout API }
  - { id: pp, col: 2, row: 1, kind: service, name: Payments pool }
  - { id: ip, col: 2, row: 2, kind: service, name: Inventory pool }
  - { id: rp, col: 2, row: 3, kind: service, name: Recs pool }
  - { id: pay, col: 3, row: 1, kind: service, name: Payments }
  - { id: inv, col: 3, row: 2, kind: service, name: Inventory }
  - { id: rec, col: 3, row: 3, kind: service, name: "Recs (hung)" }
edges:
  - { from: api, to: pp }
  - { from: api, to: ip }
  - { from: api, to: rp }
  - { from: pp, to: pay }
  - { from: ip, to: inv }
  - { from: rp, to: rec, kind: error }`,
  ),
  timeout: fence(
    'sequence',
    `title: Deadline on a slow call
actors:
  - { id: C, name: Checkout }
  - { id: I, name: Inventory API }
messages:
  - { from: C, to: I, label: reserve stock, kind: sync }
  - { from: C, to: C, label: "800ms deadline passes — abort", kind: note }
  - { from: I, to: C, label: "504 gateway timeout", kind: error }
  - { from: C, to: C, label: "fallback: cached availability", kind: note }`,
  ),
  'cache-aside': fence(
    'sequence',
    `title: Miss, load, populate
actors:
  - { id: A, name: App }
  - { id: R, name: Redis }
  - { id: D, name: Postgres }
messages:
  - { from: A, to: R, label: "GET user:42", kind: sync }
  - { from: R, to: A, label: miss, kind: response }
  - { from: A, to: D, label: SELECT user 42, kind: sync }
  - { from: D, to: A, label: row, kind: response }
  - { from: A, to: R, label: "SET user:42 TTL 5m", kind: async }`,
  ),
  throttling: fence(
    'flow',
    `title: Spend a token or shed
nodes:
  - { id: req, col: 1, row: 1, kind: start, label: Upload request }
  - { id: tok, col: 2, row: 1, kind: decision, label: "Token in bucket?" }
  - { id: fwd, col: 3, row: 1, kind: end, label: "Consume + process" }
  - { id: shed, col: 2, row: 2, kind: end, label: "429 Retry-After" }
  - { id: refill, col: 3, row: 2, kind: process, label: "Refill 100/s" }
edges:
  - { from: req, to: tok }
  - { from: tok, to: fwd, label: "yes" }
  - { from: tok, to: shed, label: "no", kind: error }
  - { from: refill, to: tok, label: steady drip }`,
  ),
  'actor-model': fence(
    'block',
    `title: Mailboxes, not locks
groups:
  - { id: sys, col: 2, row: 1, cols: 2, rows: 2, label: Actor system, color: "#6b7280" }
nodes:
  - { id: sender, col: 1, row: 1, kind: client, name: Sender }
  - { id: mb1, col: 2, row: 1, kind: queue, name: Mailbox }
  - { id: oa, col: 3, row: 1, kind: service, name: OrderActor }
  - { id: mb2, col: 2, row: 2, kind: queue, name: Mailbox }
  - { id: sa, col: 3, row: 2, kind: service, name: StockActor }
  - { id: sup, col: 4, row: 1, kind: service, name: Supervisor }
edges:
  - { from: sender, to: mb1, label: tell }
  - { from: mb1, to: oa, label: one at a time }
  - { from: oa, to: mb2, label: message }
  - { from: mb2, to: sa }
  - { from: sup, to: oa, label: restart, kind: dashed }`,
  ),
  'producer-consumer': fence(
    'event',
    `title: Bounded hand-off
groups:
  - { id: ps, col: 1, row: 1, cols: 1, rows: 3, label: Producers, color: "#1a6dbe" }
  - { id: cs, col: 3, row: 1, cols: 1, rows: 3, label: Consumers, color: "#1f9747" }
nodes:
  - { id: up, col: 1, row: 1, kind: producer, name: Upload API }
  - { id: imp, col: 1, row: 3, kind: producer, name: Import job }
  - { id: buf, col: 2, row: 2, kind: queue, name: "Buffer (cap 1k)" }
  - { id: thumb, col: 3, row: 1, kind: consumer, name: Thumbnailer }
  - { id: scan, col: 3, row: 3, kind: consumer, name: Virus scan }
edges:
  - { from: up, to: buf, label: push }
  - { from: imp, to: buf, label: block when full }
  - { from: buf, to: thumb, label: pull }
  - { from: buf, to: scan, label: pull }`,
  ),
  'thread-pool': fence(
    'block',
    `title: Queue feeding a fixed pool
groups:
  - { id: pool, col: 3, row: 1, cols: 2, rows: 2, label: "Pool (4 workers)", color: "#6b7280" }
nodes:
  - { id: sub, col: 1, row: 1, kind: client, name: Submitters }
  - { id: q, col: 2, row: 1, kind: queue, name: Task queue }
  - { id: w1, col: 3, row: 1, kind: service, name: Worker 1 }
  - { id: w2, col: 4, row: 1, kind: service, name: Worker 2 }
  - { id: w3, col: 3, row: 2, kind: service, name: Worker 3 }
  - { id: w4, col: 4, row: 2, kind: service, name: Worker 4 }
edges:
  - { from: sub, to: q, label: submit }
  - { from: q, to: w1, label: next task }
  - { from: q, to: w3 }`,
  ),
  'competing-consumers': fence(
    'event',
    `title: First taker wins
groups:
  - { id: cc, col: 3, row: 1, cols: 1, rows: 3, label: Competing consumers, color: "#6b7280" }
nodes:
  - { id: ord, col: 1, row: 2, kind: producer, name: Order service }
  - { id: q, col: 2, row: 2, kind: queue, name: orders queue }
  - { id: wa, col: 3, row: 1, kind: consumer, name: Worker A }
  - { id: wb, col: 3, row: 2, kind: consumer, name: Worker B }
  - { id: wc, col: 3, row: 3, kind: consumer, name: Worker C }
edges:
  - { from: ord, to: q, label: publish }
  - { from: q, to: wa, kind: dashed }
  - { from: q, to: wb, label: delivered once }
  - { from: q, to: wc, kind: dashed }`,
  ),
  'splitter-aggregator': fence(
    'dag',
    `title: Split, fan out, recombine
nodes:
  - { id: order, col: 1, row: 2, kind: start, label: Bulk order }
  - { id: split, col: 2, row: 2, kind: process, label: Splitter }
  - { id: books, col: 3, row: 1, kind: process, label: Books line }
  - { id: elec, col: 3, row: 2, kind: process, label: Electronics line }
  - { id: groc, col: 3, row: 3, kind: process, label: Grocery line }
  - { id: agg, col: 4, row: 2, kind: process, label: Aggregator }
  - { id: conf, col: 5, row: 2, kind: end, label: One confirmation }
edges:
  - { from: order, to: split }
  - { from: split, to: books }
  - { from: split, to: elec }
  ...
  - { from: elec, to: agg }
  - { from: groc, to: agg }
  - { from: agg, to: conf, label: all parts in }`,
  ),
  // ── AI / agents ────────────────────────────────────────────────────────────
  rag: fence(
    'sequence',
...
edges:
  ...
  'plan-and-execute': fence(
    'flow',
    `title: Plan, execute, replan
nodes:
  - { id: goal, col: 1, row: 1, kind: start, label: Goal }
  - { id: plan, col: 2, row: 1, kind: process, label: Draft step plan }
  - { id: exec, col: 3, row: 1, kind: process, label: Execute next step }
  - { id: more, col: 4, row: 1, kind: decision, label: "Steps left?" }
  - { id: track, col: 3, row: 2, kind: decision, label: "On track?" }
  - { id: done, col: 5, row: 1, kind: end, label: Result }
edges:
  - { from: goal, to: plan }
  - { from: plan, to: exec }
  - { from: exec, to: more }
  - { from: more, to: done, label: "no" }
  - { from: more, to: track, label: "yes" }
  - { from: track, to: exec, label: "yes" }
  - { from: track, to: plan, label: "no, replan", kind: error }`,
  ),
  'human-in-the-loop': fence(
    'swimlane',
    `title: Approval gate
lanes:
  - { label: Agent }
...
steps:
  - { id: draft, col: 1, lane: 0, label: Draft refund, kind: start }
  - { id: risk, col: 2, lane: 0, label: "Risky?", kind: decision }
  - { id: auto, col: 3, lane: 0, label: Execute directly, kind: action }
  - { id: review, col: 3, lane: 1, label: Review proposal, kind: action }
  - { id: appr, col: 4, lane: 1, label: "Approve?", kind: decision }
  - { id: exec, col: 5, lane: 0, label: Issue refund, kind: end }
  - { id: rej, col: 5, lane: 1, label: Return feedback, kind: end }
links:
  - { from: draft, to: risk }
  - { from: risk, to: auto, label: low risk }
  - { from: risk, to: review, label: needs approval }
  - { from: review, to: appr }
  - { from: appr, to: exec, label: "yes" }
  - { from: appr, to: rej, label: "no" }`,
  ),
  'agentic-rag': fence(
    'sequence',
    `title: Retrieval as a tool, on demand
actors:
  - { id: U, name: User }
  - { id: A, name: Agent }
  - { id: S, name: Search tool }
messages:
  - { from: U, to: A, label: "why did Q3 churn spike?", kind: sync }
  - { from: A, to: A, label: "decides: needs evidence", kind: note }
  - { from: A, to: S, label: "search(churn Q3)", kind: sync }
  - { from: S, to: A, label: top chunks, kind: response }
  - { from: A, to: A, label: "grades: pricing angle missing", kind: note }
  - { from: A, to: S, label: "search(pricing change Q3)", kind: sync }
  - { from: S, to: A, label: more chunks, kind: response }
  - { from: A, to: U, label: grounded answer, kind: response }`,
  ),
  'swarm-handoff': fence(
    'block',
    `title: Conversation passed between specialists
nodes:
  - { id: cust, col: 1, row: 2, kind: client, name: Customer }
  - { id: triage, col: 2, row: 2, kind: service, name: Triage agent }
  - { id: bill, col: 3, row: 1, kind: service, name: Billing agent }
  - { id: tech, col: 3, row: 3, kind: service, name: Tech agent }
  - { id: ctx, col: 4, row: 2, kind: store, name: Conversation state }
edges:
  - { from: cust, to: triage, label: chat }
  - { from: triage, to: bill, label: "handoff + context" }
  - { from: bill, to: tech, label: "handoff + context" }
  - { from: tech, to: triage, label: hand back, kind: dashed }
  - { from: bill, to: ctx, kind: dashed }
  - { from: tech, to: ctx, kind: dashed, label: reads }`,
  ),
  'chain-of-thought': fence(
    'prompt',
    `title: Show the work before the answer
segments:
  - kind: system
    text: "Reason step by step. Finish with one line starting with Answer:."
  - kind: user
    label: Worked example
    text: "Q: A warehouse ships {{units}} units across {{trucks}} trucks. How many per truck?"
  - kind: assistant
    text: "Each truck carries units / trucks = {{units}} / {{trucks}} = 40. Answer: 40"
  - kind: user
    text: "Q: {{question}}"
vars:
  - { name: units, desc: Quantity in the worked example }
  - { name: trucks, desc: Divisor in the worked example }
  - { name: question, desc: The new multi-step problem to solve }`,
  ),
  'context-compaction': fence(
    'gallery',
    `title: The window, before and after
cols: 2
items:
  - title: "Turn 40 — nearly full"
    caption: Stale history crowds out the budget.
    block:
      type: context
      window: 200000
      unit: tokens
      segments:
        - { label: System prompt, tokens: 3000, accent: navy }
        - { label: Tool definitions, tokens: 9000, accent: purple }
        - { label: "Turns 1-35", tokens: 150000, accent: amber, desc: stale history }
        - { label: "Turns 36-40", tokens: 30000, accent: green }
  - title: After compaction
    caption: Old turns collapse into a 4k summary.
    block:
      type: context
      window: 200000
      unit: tokens
      segments:
        - { label: System prompt, tokens: 3000, accent: navy }
        - { label: Tool definitions, tokens: 9000, accent: purple }
        - { label: "Summary of turns 1-35", tokens: 4000, accent: amber }
        - { label: "Turns 36-40", tokens: 30000, accent: green }`,
  ),
  // ── creational (uml) ───────────────────────────────────────────────────────
...
rels:
  ...
  interpreter: fence(
    ...
    `title: Interpreter
classes:
  - { id: expr, col: 2, row: 1, name: AbstractExpression, stereotype: interface, methods: ["interpret(ctx)"] }
  - { id: ctx, col: 3, row: 1, name: Context, attrs: ["variables"] }
  - { id: term, col: 1, row: 2, name: TerminalExpression, methods: ["interpret(ctx)"] }
  - { id: nonterm, col: 3, row: 2, name: NonterminalExpression, methods: ["interpret(ctx)"] }
rels:
  - { from: term, to: expr, kind: implementation }
  - { from: nonterm, to: expr, kind: implementation }
  - { from: nonterm, to: expr, kind: aggregation, label: children }
  - { from: expr, to: ctx, kind: dependency, label: reads }`,
  ),
  // ── architecture ────────────────────────────────────────────────────────────
  mvc: fence(
    'belogic',
    `title: The MVC triangle
description: The controller mutates the model; the view observes it and re-renders.
nodes:
  - { id: ctl, col: 1, row: 1, kind: controller, name: Controller, note: handles input }
  - { id: model, col: 3, row: 1, kind: model, name: Model, note: "state + rules" }
  - { id: view, col: 2, row: 2, kind: view, name: View, note: renders the model }
edges:
  - { from: ctl, to: model, kind: uses, label: mutates }
  - { from: view, to: model, kind: reads, label: observes }
  - { from: ctl, to: view, kind: uses, label: selects }`,
  ),
  mvvm: fence(
    'belogic',
    `title: Bindable view state
description: The view binds to a view-model that knows nothing about the view.
nodes:
  - { id: view, col: 1, row: 1, kind: view, name: CheckoutView, note: declarative markup }
  - { id: vm, col: 2, row: 1, kind: service, name: CheckoutViewModel, note: "state + commands" }
  - { id: model, col: 3, row: 1, kind: model, name: Cart, note: domain rules }
edges:
  - { from: view, to: vm, kind: reads, label: two-way binding }
  - { from: vm, to: model, kind: uses }`,
  ),
  'dependency-injection': fence(
    'uml',
    `title: Injected, not constructed
classes:
  - { id: root, col: 1, row: 1, name: CompositionRoot, methods: ["wire()"] }
  - { id: svc, col: 2, row: 1, name: NotificationService, attrs: ["- sender: MessageSender"] }
  - { id: iface, col: 3, row: 1, name: MessageSender, stereotype: interface, methods: ["send(msg)"] }
  - { id: email, col: 3, row: 2, name: EmailSender }
  - { id: fake, col: 2, row: 2, name: FakeSender }
rels:
  - { from: svc, to: iface, kind: aggregation, label: injected }
  - { from: email, to: iface, kind: implementation }
  - { from: fake, to: iface, kind: implementation }
  - { from: root, to: svc, kind: dependency, label: constructs }
  - { from: root, to: email, kind: dependency, label: constructs }`,
  ),
  'unit-of-work': fence(
    'sequence',
    `title: Track everything, commit once
actors:
  - { id: S, name: OrderService }
  - { id: U, name: UnitOfWork }
  - { id: D, name: Database }
messages:
  - { from: S, to: U, label: registerNew(order), kind: sync }
  - { from: S, to: U, label: registerDirty(inventory), kind: sync }
  - { from: S, to: U, label: commit(), kind: sync }
  - { from: U, to: D, label: "BEGIN, INSERT, UPDATE, COMMIT", kind: sync }
  - { from: D, to: U, label: ok, kind: response }
  - { from: U, to: S, label: done, kind: response }`,
  ),
  'active-record': fence(
    'uml',
    `title: Rows that save themselves
classes:
  - { id: base, col: 2, row: 1, name: ActiveRecord, stereotype: abstract, methods: ["find(id)", "save()", "delete()"] }
  - { id: user, col: 1, row: 2, name: User, attrs: ["name", "email"], methods: ["deactivate()"] }
  - { id: order, col: 3, row: 2, name: Order, attrs: ["total", "status"], methods: ["markPaid()"] }
rels:
  - { from: user, to: base, kind: inheritance }
  - { from: order, to: base, kind: inheritance }
  - { from: user, to: order, kind: association, label: one row each }`,
  ),
  'data-mapper': fence(
    'belogic',
    `title: Persistence-ignorant domain
description: The mapper owns all SQL; the Invoice object never sees the database.
nodes:
  - { id: svc, col: 1, row: 1, kind: service, name: BillingService, note: asks for Invoice 42 }
  - { id: dom, col: 2, row: 2, kind: model, name: Invoice, note: "pure behavior, no SQL" }
  - { id: map, col: 2, row: 1, kind: repository, name: InvoiceMapper, note: toDomain / toRow }
  - { id: db, col: 3, row: 1, kind: db, name: Postgres, note: invoices table }
edges:
  - { from: svc, to: map, kind: uses, label: find(42) }
  - { from: map, to: db, kind: egress, label: SELECT }
  - { from: map, to: dom, kind: uses, label: hydrates }
  - { from: svc, to: dom, kind: reads }`,
  ),
  'event-bus': fence(
    'belogic',
    `title: Modules that never import each other
description: Checkout publishes a typed event; subscribers react without any direct link.
nodes:
  - { id: checkout, col: 1, row: 1, kind: service, name: CheckoutModule, note: emits OrderPlaced }
  - { id: bus, col: 2, row: 1, kind: bus, name: EventBus, note: subscribe / publish }
  - { id: mail, col: 3, row: 1, kind: worker, name: EmailModule, note: sends receipt }
  - { id: loyal, col: 3, row: 2, kind: worker, name: LoyaltyModule, note: awards points }
edges:
  - { from: checkout, to: bus, kind: async, label: publish OrderPlaced }
  - { from: bus, to: mail, kind: async }
  - { from: bus, to: loyal, kind: async }`,
  ),
  specification: fence(
    'uml',
    `title: Composable business rules
classes:
  - { id: spec, col: 2, row: 1, name: Specification, stereotype: interface, methods: ["isSatisfiedBy(c)", "and(other)", "not()"] }
  - { id: gold, col: 1, row: 2, name: GoldCustomerSpec }
  - { id: overdue, col: 2, row: 2, name: OverdueInvoiceSpec }
  - { id: comp, col: 3, row: 2, name: AndSpecification, attrs: ["left", "right"] }
rels:
  - { from: gold, to: spec, kind: implementation }
  - { from: overdue, to: spec, kind: implementation }
  - { from: comp, to: spec, kind: implementation }
  - { from: comp, to: spec, kind: aggregation, label: composes two }`,
  ),
  'null-object': fence(
    'uml',
    `title: A safe stand-in for null
classes:
  - { id: svc, col: 1, row: 1, name: ReportService, attrs: ["logger"] }
  - { id: log, col: 2, row: 1, name: Logger, stereotype: interface, methods: ["log(msg)"] }
  - { id: file, col: 2, row: 2, name: FileLogger, methods: ["log appends"] }
  - { id: nul, col: 3, row: 2, name: NullLogger, methods: ["log does nothing"] }
rels:
  - { from: svc, to: log, kind: aggregation, label: never null }
  - { from: file, to: log, kind: implementation }
  - { from: nul, to: log, kind: implementation }`,
  ),
};
 ...
 *
 * In `slides` mode each section is kept light — heading + one-line intent + the
 * diagram (the star of a slide) — so the deck reads cleanly instead of cramming
 * the full card and the diagram onto one slide. The full card stays in the HTML
 * gallery and in `avo design <slug>`. A pattern with no diagram falls back to its
 * card.
 */
export function buildDesignDoc(filter?: DesignFilter, slides = false): string {
  ...
  const sections = picked.map((p) => {
    const structure = structureBlock(p);
    if (slides) {
      const body = structure !== '' ? structure : patternBlock(p);
      return `## ${p.name}\n\n${p.summary}\n\n${body}`;
    }
...
export async function runDesignGallery(opts: {
  ...
  await writeFile(input, buildDesignDoc(opts.filter, format === 'slides'), 'utf8');
  return runSingle({
    cwd: dir,
    input,
    format,
