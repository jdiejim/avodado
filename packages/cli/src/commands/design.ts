...
clusters:
  - { id: ctrl, label: control plane, kind: namespace }
  - { id: app, label: app namespace, kind: namespace }
services:
  - { id: istiod, cluster: ctrl, label: control plane, kind: service, tech: istiod, replicas: 1 }
  - { id: a, cluster: app, label: orders, kind: service, tech: Go, replicas: 3 }
  - { id: pa, cluster: app, label: sidecar, kind: service, tech: Envoy, replicas: 3 }
  - { id: b, cluster: app, label: payments, kind: service, tech: Java, replicas: 2 }
  - { id: pb, cluster: app, label: sidecar, kind: service, tech: Envoy, replicas: 2 }
edges:
  - { from: a, to: pa }
  - { from: b, to: pb }
  - { from: pa, to: pb, label: mTLS }
  - { from: istiod, to: pa, kind: dashed }
  - { from: istiod, to: pb, kind: dashed }`,
  ),
  'strangler-fig': fence(
    'flow',
