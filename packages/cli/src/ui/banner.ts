...
/** A small cute ASCII avocado (green skin, amber pit), based on the logo. */
export function avocadoArt(): string {
  const g = pc.green;
  return [
    '   ' + g('.-""-.'),
    '  ' + g('/      \\'),
...
/**
 * A fun per-command header: the ASCII avocado next to the action word rendered
 * big in avocado-green via cfonts (e.g. "slides", "preview", "export").
 */
export function actionBanner(word: string): string {
  ...
  try {
    ...
    /* fall back to just the avocado */
  }
  const avo = avocadoArt().split('\n');
  const word2 = (art !== '' ? art : pc.green(pc.bold(word))).split('\n');
  // Place the avocado on the left, the word to its right, vertically centered.
  const pad = '  ';
  const offset = Math.max(0, Math.floor((avo.length - word2.length) / 2));
  const rows = Math.max(avo.length, word2.length);
  const lines: string[] = [];
  for (let i = 0; i < rows; i++) {
    const left = avo[i] ?? '          ';
    const right = i - offset >= 0 && i - offset < word2.length ? word2[i - offset] : '';
