...
/** The grouped command epilogue, shown after top-level help. */
export function examples(): string {
  const rows = HELP_GROUPS.map(
    (g) =>
      `  ${pc.dim(g.header.padEnd(9))}${g.commands.map((c) => pc.cyan(c)).join(pc.dim(' · '))}`,
  );
  return [
    '',
    ...rows,
    '',
    `  ${pc.dim('New here?')} avo ${pc.cyan('tour')} ${pc.dim('· per-command help:')} avo ${pc.cyan('<command>')} --help`,
