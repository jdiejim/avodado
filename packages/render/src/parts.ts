  ...
  if (usesHeadings) {
    ...
    const flush = (): void => {
      ...
      if (html.trim() !== '' || heading !== undefined) {
        // A heading marker ({top}/{center}/{bottom}) wins; otherwise auto: light
        // slides (≤1 block, little prose) center, heavier ones top-align.
    ...
    for (const seg of doc.segments) {
      ...
      if (seg.kind === 'markdown') {
        ...
        for (const line of seg.text.split('\n')) {
          ...
          if (m !== null) {
            flushBuf();
            flush(); // a heading starts a new slide
