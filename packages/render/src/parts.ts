...
export function renderSlides(doc: Document, opts: RenderPartsOptions = {}): SlidesResult {
  ...
  if (usesHeadings) {
    ...
    const pushSlide = (keep: boolean): void => {
      ...
      if (html.trim() !== '' || (heading !== undefined && !keep)) {
        const align = forced ?? (blocks >= 2 || proseChars > 240 ? 'top' : 'center');
        slides.push({
          label: label ?? 'Slide',
    ...
    // would overflow the budget (but never flush an empty slide).
    const addPart = (h: string, w: number): void => {
      if (parts.length > 0 && weight + w > SLIDE_BUDGET) pushSlide(true);
      parts.push(h);
      weight += w;
    };
      ...
      if (seg.kind === 'markdown') {
        ...
        const flushBuf = (): void => {
          ...
          if (text !== '') {
            ...
            if (h.trim() !== '') {
              addPart(h, Math.max(1, text.length / 180));
        ...
        for (const line of seg.text.split('\n')) {
          ...
          if (m !== null) {
            ...
            // Optional alignment marker, e.g. `## Title {top}` — stripped from the title.
            let title = m[2] ?? '';
            const mark = /\s*\{(top|center|middle|bottom)\}\s*$/i.exec(title);
            if (mark !== null) {
        ...
        addPart(renderSegment(seg, ctx), blockWeight(seg));
        blocks += 1;
