...
export function renderSlides(doc: Document, opts: RenderPartsOptions = {}): SlidesResult {
  ...
  // stays on that slide. A `---` thematic break also starts a new slide. `###`+
  // headings stay in the body. If the doc has no headings or breaks at all, we
  // fall back to one slide per block (legacy behavior).
  const headingRe = /^(#{1,2})\s+(.+?)\s*$/;
  const hrRe = /^\s*---\s*$/;
  const usesBreaks = doc.segments.some(
    (s) => s.kind === 'markdown' && s.text.split('\n').some((l) => headingRe.test(l) || hrRe.test(l)),
  );
