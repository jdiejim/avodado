...
export function renderSlides(doc: Document, opts: RenderPartsOptions = {}): SlidesResult {
  ...
  for (const seg of doc.segments) {
    if (seg.kind === 'markdown') {
      // A top-level Markdown heading (`#`/`##`) becomes the slide's title at the
      // top — like the source — so it isn't repeated in the slide body. Deeper
      // headings (`###`+) stay in the body.
      const kept: string[] = [];
    ...
    // Prefer the Markdown heading as the slide title; fall back to the block's title.
