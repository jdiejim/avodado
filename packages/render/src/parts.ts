...
export interface Slide {
  ...
  /** Vertical alignment of the content: `center` for light slides, `top` for heavy ones. */
  ...
  if (usesHeadings) {
    ...
    const flush = (): void => {
      ...
      if (html.trim() !== '' || heading !== undefined) {
        // Light slides (≤1 block, little prose) read better centered; heavier
        // slides (stacked blocks or lots of prose) read better top-aligned.
        const align: 'top' | 'center' = blocks >= 2 || proseChars > 240 ? 'top' : 'center';
        slides.push({
          label: label ?? 'Slide',
