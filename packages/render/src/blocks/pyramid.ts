...
export function renderPyramid(data: BlockDataMap['pyramid']): string {
  ...
  const polygons = levels
    .map((L, i) => {
      ...
      const labelEls = lines
        ...
        .join('');
      const desc = hasDesc
        ? `<text x="${apex}" y="${y + rowH / 2 + 14}" class="pyr-desc"${bp(`levels.${i}.desc`)}>${escapeHtml(L.desc ?? '')}</text>`
        : '';
      return (
        `<g${bp(`levels.${i}`)}>` +
  ...
  return `<svg viewBox="0 0 ${W} ${H}" role="img"><title>Pyramid</title><g${bl('levels')}>${polygons}</g></svg>`;
}
