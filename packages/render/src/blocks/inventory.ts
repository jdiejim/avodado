...
function renderRow(item: InventoryItem, i: number): string {
  const tag =
    item.tag !== undefined
      ? `<span class="inv-tag"${bp(`items.${i}.tag`)}>${escapeHtml(item.tag)}</span>`
      : '';
  const note =
    item.note !== undefined
      ? `<div class="inv-note"${bp(`items.${i}.note`)}>${escapeHtml(item.note)}</div>`
      : '';
  return (
    `<div class="inv-row"${bp(`items.${i}`)}>` +
    `<div class="inv-main">` +
    `<div class="inv-top"><span class="inv-name"${bp(`items.${i}.name`)}>${escapeHtml(item.name)}</span>${tag}</div>` +
    note +
    `</div>` +
    `<span class="inv-status ${(STATUS_CLASS as Partial<Record<string, string>>)[item.status] ?? ''}"${bp(`items.${i}.status`)}>${escapeHtml(item.status)}</span>` +
    `</div>`
  );
}
...
export function renderInventory(data: InventoryData): string {
  ...
  const desc =
    data.description !== undefined
      ...
      : '';
  const rows = data.items.map((item, i) => renderRow(item, i)).join('');
  return `<div class="inventory">${head}${desc}<div class="inv-list"${bl('items')}>${rows}</div></div>`;
}
