    ...
    .map((item, i) => {
      const detail =
        item.detail !== undefined
          ? `<p class="tk-detail"${bp(`items.${i}.detail`)}>${escapeHtml(item.detail)}</p>`
          : '';
      return (
        `<li class="tk-item"${bp(`items.${i}`)}>` +
        `<span class="tk-num" aria-hidden="true">${i + 1}</span>` +
        `<div class="tk-body"><div class="tk-text"${bp(`items.${i}.text`)}>${escapeHtml(item.text)}</div>${detail}</div>` +
        `</li>`
      );
    })
    .join('');
  return `<div class="tk${accentCls}"><div class="tk-title">${title}</div><ol class="tk-list"${bl('items')}>${rows}</ol></div>`;
}
