...
export function renderScorecard(data: ScorecardData): string {
  ...
  const headCells = options
    .map((opt, i) => {
      ...
      const note =
        opt.note !== undefined
          ? `<span class="sc-note"${bp(`options.${i}.note`)}>${escapeHtml(opt.note)}</span>`
          : '';
      return `<th class="c${win ? ' sc-win' : ''}"${bp(`options.${i}`)}>${escapeHtml(opt.label)}${chip}${note}</th>`;
    })
    .join('');
  const head = `<tr${bl('options')}><th>Criteria</th>${headCells}</tr>`;
  ...
  const body = criteria
    .map((c, ci) => {
      ...
      const chip =
        weight !== 1
          ? `<span class="sc-weight"${bp(`criteria.${ci}.weight`)}>×${escapeHtml(String(weight))}</span>`
          : '';
      const cells = options
        .map((opt, oi) => {
          const score = opt.scores[ci];
          return `<td class="c sc-score"${bp(`options.${oi}.scores.${ci}`)}>${score !== undefined ? escapeHtml(String(score)) : '—'}</td>`;
        })
        .join('');
      return `<tr${bp(`criteria.${ci}`)}><td class="sc-crit"${bp(`criteria.${ci}.label`)}>${escapeHtml(c.label)}${chip}</td>${cells}</tr>`;
    })
    .join('');
  ...
  return (
    ...
    `<thead>${head}</thead><tbody${bl('criteria')}>${body}${foot}</tbody>` +
    `</table></div></div>`
  );
}
