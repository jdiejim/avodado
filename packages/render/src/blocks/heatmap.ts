...
export function renderHeatmap(data: HeatmapData): string {
  ...
  grid += cols.map((c, i) => `<div class="hm-col"${bp(`xLabels.${i}`)}>${escapeHtml(c)}</div>`).join('');
  rows.forEach((row, ri) => {
    grid += `<div class="hm-rowlabel"${bp(`rows.${ri}.label`)}>${escapeHtml(row.label)}</div>`;
    for (let i = 0; i < cols.length; i++) {
      ...
      grid += `<div class="hm-cell${dark}" style="background:${rampAt(t)}"${title}${bp(`rows.${ri}.values.${i}`)}>${escapeHtml(fmt(v))}</div>`;
    }
  });
