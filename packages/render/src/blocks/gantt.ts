...
export function renderGantt(data: BlockDataMap['gantt']): string {
  ...
  const colW = 64;
  const padBot = 14;
  // Long task labels wrap (≤2 lines) inside the label gutter instead of
  // running under the bars; every row grows uniformly when any label wraps.
  const labelLines = tasks.map((t) => wrapText(t.label, Math.floor((labelW - padX) / 6.2), 3));
  const maxLines = Math.max(1, ...labelLines.map((ls) => ls.length));
  const rowH = 30 + (maxLines - 1) * 13;
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    const label =
      ls.length <= 1
        ? `<text x="${padX}" y="${y + rowH / 2 + 4}" class="gantt-label"${bp(`tasks.${i}.label`)}>${escapeHtml(t.label)}</text>`
        : `<g${bp(`tasks.${i}.label`)}>` +
          ls
            .map(
              (ln, j) =>
                `<text x="${padX}" y="${y + rowH / 2 + 4 - (ls.length - 1) * 6.5 + j * 13}" class="gantt-label">${escapeHtml(ln)}</text>`,
            )
            .join('') +
          `</g>`;
    s +=
