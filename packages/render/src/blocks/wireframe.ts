...
function drawScreen(screen: Screen, idx: number): { svg: string; width: number; height: number } {
  ...
  if (isPhone) {
    ...
    if (screen.title)
      inner += `<text x="16" y="20" class="wf-status"${bp(`screens.${idx}.title`)}>${escapeHtml(screen.title)}</text>`;
  ...
  inner += `<g${bl(`screens.${idx}.elements`)}>`;
  els.forEach((el, j) => {
    inner += `<g${bp(`screens.${idx}.elements.${j}`)}>${drawElement(el, pad, cy, cw)}</g>`;
    cy += elementHeight(el) + 10;
  });
  inner += `</g>`; // close the elements list container
  if (isPhone) {
...
export function renderWireframe(data: BlockDataMap['wireframe']): string {
  ...
  s += `</g>`; // close the screens list container
  s += `</svg>`;
