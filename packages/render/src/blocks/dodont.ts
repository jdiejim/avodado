...
function renderItem(item: DodontItem, listPath: 'dos' | 'donts', i: number): string {
  const example =
    item.example !== undefined
      ? `<div class="dd-ex"${bp(`${listPath}.${i}.example`)}>${escapeHtml(item.example)}</div>`
      : '';
  return `<div class="dd-item"${bp(`${listPath}.${i}`)}><div class="dd-text"${bp(`${listPath}.${i}.text`)}>${escapeHtml(item.text)}</div>${example}</div>`;
}
...
function renderCard(
  ...
  return (
    ...
    `</div>` +
    `<div class="dd-list"${bl(listPath)}>${items.map((item, i) => renderItem(item, listPath, i)).join('')}</div>` +
    `</div>`
  );
}
...
export function renderDodont(data: DodontData): string {
  ...
  return (
    ...
    `<div class="dd-grid">` +
    renderCard('dd-do', '✓', 'Do', data.dos, 'dos') +
    renderCard('dd-dont', '✕', "Don't", data.donts, 'donts') +
    `</div>` +
    `</div>`
  );
}
