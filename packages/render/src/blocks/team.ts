...
function renderMember(m: Member, i: number): string {
  ...
  const role =
    m.role !== undefined
      ? `<div class="tem-role"${bp(`members.${i}.role`)}>${escapeHtml(m.role)}</div>`
      : '';
  const focus =
    m.focus !== undefined
      ? `<div class="tem-focus"${bp(`members.${i}.focus`)}>${escapeHtml(m.focus)}</div>`
      : '';
  return (
    `<div class="tem-card${accent}"${bp(`members.${i}`)}>` +
    `<span class="tem-avatar">${escapeHtml(initials)}</span>` +
    `<div class="tem-name"${bp(`members.${i}.name`)}>${escapeHtml(m.name)}</div>` +
    role +
    focus +
    `</div>`
  );
...
export function renderTeam(data: TeamData): string {
  ...
  const desc =
    data.description !== undefined
      ...
      : '';
  const members = data.members.map((m, i) => renderMember(m, i)).join('');
  return `<div class="team">${head}${desc}<div class="tem-grid"${bl('members')}>${members}</div></div>`;
}
