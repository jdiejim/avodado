...
export function renderState(data: BlockDataMap['state']): string {
  ...
  for (const t of trans) {
    ...
    if (t.from === t.to) {
      ...
      s += `<path d="M ${r.cx - 12} ${r.y} C ${r.cx - 30} ${r.y - 32}, ${r.cx + 30} ${r.y - 32}, ${r.cx + 12} ${r.y}" fill="none" stroke="var(--charcoal)" stroke-width="1.3" marker-end="url(#gArrow)"/>`;
    ...
    s += `<path d="${p.d}" fill="none" stroke="var(--charcoal)" stroke-width="1.3" marker-end="url(#gArrow)"/>`;
    labels.push(edgePill(p, label));
  }
  ...
  for (const st of states) {
    ...
    if (st.kind === 'start') {
      s += `<circle cx="${r.cx}" cy="${r.cy}" r="10" fill="var(--charcoal)"/>`;
      ...
      s +=
        ...
        `<text x="${r.cx}" y="${r.cy + 4.5}" class="sm-name" fill="var(--charcoal)">${escapeHtml(st.name ?? '')}</text>` +
        `</g>`;
    }
  }
