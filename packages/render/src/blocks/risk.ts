...
function renderItem(item: RiskItem, i: number): string {
  const sev = severityOf(item);
  const mitigation =
    item.mitigation !== undefined
      ? `<p class="rk-mitigation"${bp(`items.${i}.mitigation`)}><span class="rk-mit-label">Mitigation:</span> ${escapeHtml(item.mitigation)}</p>`
      : '';
  const owner =
    item.owner !== undefined
      ? `<span class="rk-owner"${bp(`items.${i}.owner`)}>${escapeHtml(item.owner)}</span>`
      : '';
  const status =
    item.status !== undefined
      ? `<span class="rk-status rk-st-${item.status}"${bp(`items.${i}.status`)}>${escapeHtml(item.status)}</span>`
      : '';
  const chips = owner !== '' || status !== '' ? `<span class="rk-chips">${owner}${status}</span>` : '';
  return (
    `<div class="rk-item"${bp(`items.${i}`)}>` +
    `<div class="rk-top">` +
    `<span class="rk-sev rk-sev-${sev}">${sev}</span>` +
    `<span class="rk-risk"${bp(`items.${i}.risk`)}>${escapeHtml(item.risk)}</span>` +
    chips +
    `</div>` +
...
export function renderRisk(data: RiskData): string {
  ...
  return `<div class="risk">${head}${desc}<div class="rk-list"${bl('items')}>${items}</div></div>`;
}
