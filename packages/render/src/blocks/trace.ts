...
function renderTurn(turn: Turn, i: number): string {
  const parts: string[] = [];
  if (turn.thinking !== undefined && turn.thinking.length > 0) {
    parts.push(`<div class="tr-think"${bp(`turns.${i}.thinking`)}>${escapeHtml(turn.thinking)}</div>`);
  }
  if (turn.tool !== undefined && turn.tool.length > 0) {
    parts.push(`<div class="tr-tool-name"${bp(`turns.${i}.tool`)}>${escapeHtml(turn.tool)}</div>`);
  }
  ...
  if (turn.text !== undefined && turn.text.length > 0) {
    parts.push(`<div class="tr-text"${bp(`turns.${i}.text`)}>${escapeHtml(turn.text)}</div>`);
  }
  return (
    `<div class="tr-turn tr-${turn.role}"${bp(`turns.${i}`)}>` +
    `<span class="tr-chip">${(ROLE_LABEL as Partial<Record<string, string>>)[turn.role] ?? escapeHtml(turn.role).toUpperCase()}</span>` +
...
export function renderTrace(data: TraceData): string {
  ...
  const desc =
    data.description !== undefined
      ...
      : '';
  const turns = data.turns.map((turn, i) => renderTurn(turn, i)).join('');
  return `<div class="trace">${head}${desc}<div class="tr-list"${bl('turns')}>${turns}</div></div>`;
}
