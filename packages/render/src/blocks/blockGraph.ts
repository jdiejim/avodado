...
function renderGrid(data: Data): string {
  ...
  // Cloud-console-style service cards: a clean white card with a coloured icon
  // badge on the left, the service name, and an optional type/tech line.
  for (const n of nodes) {
    ...
    const badgeInner =
      glyph.length > 0
        ? glyph
        : `<text x="${bx + badge / 2}" y="${by + badge / 2 + 5}" text-anchor="middle" font-family="var(--font-display)" font-size="16" font-weight="700" fill="var(--white)">${escapeHtml((n.name ?? '?').slice(0, 1).toUpperCase())}</text>`;
    const tx = bx + badge + 13;
    const hasTech = n.tech !== undefined;
    s +=
      `<g filter="url(#gshadow)">` +
      `<rect x="${r.x}" y="${r.y}" width="${r.w}" height="${r.h}" rx="12" fill="var(--white)" stroke="var(--rule)" stroke-width="1"/>` +
      `<rect x="${bx}" y="${by}" width="${badge}" height="${badge}" rx="10" fill="${st.accent}"/>` +
      badgeInner +
      `<text x="${tx}" y="${r.y + r.h / 2 + (hasTech ? -3 : 5)}" class="blk-name" fill="var(--charcoal)">${escapeHtml(n.name)}</text>` +
      (hasTech
        ? `<text x="${tx}" y="${r.y + r.h / 2 + 14}" class="blk-tech" fill="${st.accent}">${escapeHtml(n.tech)}</text>`
        : '') +
      `</g>`;
  }
