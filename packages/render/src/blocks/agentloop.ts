...
export function renderAgentloop(data: AgentloopData): string {
  ...
  if (hasTools) {
    ...
    s += `</g>`; // close the tools list container
    if (hiddenTools > 0) {
  ...
  if (hasMemory) {
    ...
    memItems.forEach((item, i) => {
      ...
      s += `<text x="${memX + Math.round(memW / 2)}" y="${memTop + 41 + i * 15}" text-anchor="middle" class="al-mem-item"${bp(`memory.${i}`)}>${escapeHtml(line)}</text>`;
    });
    s += `</g>`; // close the memory list container
  }
