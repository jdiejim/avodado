...
export function renderUml(data: BlockDataMap['uml']): string {
  ...
  validRels.forEach(({ rl, ri }, i) => {
    ...
    s += `<path d="${roundedPath(pts, 9)}" fill="none" stroke="var(--charcoal)" stroke-width="1.3" stroke-dasharray="${st.dash}"${start}${end}${bp(`rels.${ri}`)}/>`;
  ...
  classes.forEach((c, ci) => {
    ...
    const attrs =
      ...
      (c.attrs ?? [])
        .map(
          (a, j) =>
            `<text x="${r.x + 12}" y="${r.y + hh + 15 + j * rowH}" class="uml-row"${bp(`classes.${ci}.attrs.${j}`)}>${escapeHtml(a)}</text>`,
        )
        .join('') +
      `</g>`;
    const methods =
      ...
      (c.methods ?? [])
        .map(
          (m, j) =>
            `<text x="${r.x + 12}" y="${r.y + hh + aH + 15 + j * rowH}" class="uml-row"${bp(`classes.${ci}.methods.${j}`)}>${escapeHtml(m)}</text>`,
        )
        .join('') +
      `</g>`;
    ...
    s +=
      `<g filter="url(#gshadow)"${bp(`classes.${ci}`)}>` +
      `<rect x="${r.x}" y="${r.y}" width="${r.w}" height="${r.h}" rx="6" fill="var(--white)" stroke="var(--navy)" stroke-width="1.2"/>` +
      headBand +
      stereo +
      `<text x="${r.x + r.w / 2}" y="${nameY}" class="uml-name"${bp(`classes.${ci}.name`)}>${escapeHtml(c.name)}</text>` +
