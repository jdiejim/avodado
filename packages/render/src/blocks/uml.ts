...
export function renderUml(data: BlockDataMap['uml']): string {
  ...
  const padBot = 20;
  const rowH = 15;
  const headH = (c: UmlClass): number => 24 + (c.stereotype !== undefined ? 11 : 0);
  ...
  for (const rl of rels) {
    ...
    s += `<path d="${p.d}" fill="none" stroke="#1a1a2e" stroke-width="1.3" stroke-dasharray="${st.dash}"${start}${end}/>`;
    labels.push(edgePill(p, rl.label));
  }
