...
describe('block renderers — DOM signatures', () => {
  ...
  it('callout unknown tone degrades without an empty title', () => {
    const html = renderCallout({ tone: 'info' as 'note' });
    const root = parse(html);
    expect(root.querySelector('.callout-title')?.text).toBe('Note');
  });
  ...
  it('sequence unknown message kind degrades to the sync style instead of throwing', () => {
    // Invalid per schema (validate reports E_SCHEMA), but render is lenient by
    // design — it must degrade, never crash.
    const html = renderSequence({
      actors: [
        { id: 'C', name: 'Client' },
        { id: 'S', name: 'Server' },
      ],
      messages: [{ from: 'C', to: 'S', label: 'ping', kind: 'reply' as 'sync' }],
    });
    const root = parse(html);
    const line = root.querySelector('.msg-line');
    expect(line).toBeTruthy();
    expect(line?.getAttribute('marker-end')).toContain('sqArrow');
  });
  ...
  it('userstory link with a ref renders as an anchor; label-only stays a span', () => {
    const html = renderUserStory({
      role: 'shopper',
      want: 'pay quickly',
      soThat: 'I complete checkout',
      links: [
        { mode: 'sequence', label: 'Checkout flow', ref: 'orders#seq-checkout' },
        { mode: 'note', label: 'Plain' },
      ],
    });
    const root = parse(html);
    const a = root.querySelector('a.link-chip');
    expect(a?.getAttribute('data-ref')).toBe('orders#seq-checkout');
    ...
    const spans = root.querySelectorAll('span.link-chip');
    expect(spans).toHaveLength(1);
    expect(spans[0]?.text).toContain('Plain');
  });

  it('stories link with a ref renders as an anchor; label-only stays a span', () => {
    const html = renderStories({
      items: [
        {
          title: 'One-step pay',
          links: [{ ref: '#sw-seq-checkout', label: 'Checkout flow' }, { label: 'Notes' }],
        },
      ],
    });
    const root = parse(html);
    const a = root.querySelector('a.st-link');
    expect(a?.getAttribute('data-ref')).toBe('#sw-seq-checkout');
    expect(a?.getAttribute('href')).toBe('#sw-seq-checkout');
    expect(a?.text).toBe('Checkout flow');
    expect(html).toContain(
      '<a class="st-link" data-ref="#sw-seq-checkout" href="#sw-seq-checkout" data-bp="items.0.links.0">',
    );
    expect(root.querySelectorAll('span.st-link')).toHaveLength(1);
  });
  ...
  it('statustable legacy tracker items: pills carry status class; done rows get tr.done; priority + owner + due render', () => {
  ...
  it('statustable pills resolve user vocab first, then defaults; ragged rows pad; html escapes', () => {
    const html = renderStatustable({
      columns: ['Task', 'Update'],
      statuses: [
        { label: 'In Review', color: 'purple' },
        { label: 'stuck', color: 'error' }, // semantic alias → red
      ],
      rows: [
        { cells: ['A <task>', 'Going well'], status: 'in review' }, // case-insensitive match
        { cells: ['Ragged row only'], status: 'In Progress' }, // built-in default, padded
        { cells: ['Mystery', '???'], status: 'unheard-of' }, // unknown → gray fallback
        { cells: ['Vendor bug', 'Escalated'], status: 'stuck' }, // semantic → accent class
      ],
    });
    const root = parse(html);
    const pills = root.querySelectorAll('.stt-pill');
    expect(pills[0]?.classNames).toContain('stt-purple');
    expect(pills[1]?.classNames).toContain('stt-amber');
    expect(pills[2]?.classNames).toContain('stt-gray');
    expect(pills[3]?.classNames).toContain('stt-red');
    // Ragged row: padded to the 2 free columns + 1 status cell.
    const rows = root.querySelectorAll('tbody tr');
    expect(rows[1]?.querySelectorAll('td')).toHaveLength(3);
    expect(rows[1]?.querySelectorAll('td')[1]?.text).toBe('');
  ...
  it('statustable renders subtasks indented under their parent with their own pills', () => {
    const html = renderStatustable({
      ...
      rows: [
        {
          cells: ['Parent', 'Rolling up nothing'],
          status: 'at risk',
          subtasks: [
            { cells: ['Child A', 'Fine'], status: 'on track' },
            { cells: ['Child B', 'Stuck'], status: 'blocked' },
          ],
        },
        { cells: ['Solo', 'No children'], status: 'in review' },
      ],
    });
    const root = parse(html);
    // One <tbody> group per parent row (zebra alternates per group).
    const groups = root.querySelectorAll('tbody.stt-group');
    expect(groups).toHaveLength(2);
    expect(groups[0]?.querySelectorAll('tr')).toHaveLength(3);
    // Parent keeps its own explicit status — no roll-up.
    const parentPill = groups[0]?.querySelector('tr:first-child .stt-pill');
    expect(parentPill?.classNames).toContain('stt-amber');
    // Subtask rows are marked and carry the tree glyph (├ then └ for the last).
    const subs = groups[0]?.querySelectorAll('tr.stt-sub') ?? [];
    expect(subs).toHaveLength(2);
    expect(subs[0]?.querySelector('.stt-tree')?.text).toBe('├');
    expect(subs[1]?.querySelector('.stt-tree')?.text).toBe('└');
    expect(subs[1]?.querySelector('.stt-pill')?.classNames).toContain('stt-red');
    // 4 user-defined labels → the legend appears, one key per label.
    expect(root.querySelectorAll('.stt-legend .stt-key')).toHaveLength(4);
  });
