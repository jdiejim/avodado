...
// The real package version, so the MCP handshake reports it honestly. At
// runtime dist/server.js sits one level under the package root, where
// package.json ships in the published tarball.
const { version } = createRequire(import.meta.url)('../package.json') as { version: string };

const server = new McpServer({ name: 'avodado', version });
...
server.registerTool(
  'list_block_types',
  {
    ...
    description:
      'List every canonical Avodado block type, plus the permanent alias spellings (old merged names) that map to them.',
    inputSchema: {},
  },
  () => ({ content: [{ type: 'text', text: blockTypesListing() }] }),
);
