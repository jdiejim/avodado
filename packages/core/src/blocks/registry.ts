/**
 * Block registry — the architectural backbone.
 *
 * A {@link BlockDef} ties a block type to its schema and an optional reference
 * extractor. The registry itself is a mapped type over {@link BlockType}, so
 * adding a new block type without an entry is a compile error.
 *
 * Downstream rendering targets (HTML renderer, etc.) follow the same
 * `Record<BlockType, …>` pattern to inherit the same exhaustiveness guarantee.
 */

import type { BlockType } from '../types.js';
import { blockSchemas, type BlockDataMap } from './schemas.js';

/** Definition of one block type: its schema plus optional ref extraction. */
export interface BlockDef<K extends BlockType> {
  readonly type: K;
  readonly schema: (typeof blockSchemas)[K];
  /**
   * Extracts the reference strings (`doc#id` or `#id`) carried by this block's
   * data, if any. Only blocks that can reference other blocks define this.
   */
  readonly extractRefs?: (data: BlockDataMap[K]) => readonly string[];
}

/** Mapped registry type — adding a new {@link BlockType} without an entry fails tsc. */
export type BlockRegistry = { readonly [K in BlockType]: BlockDef<K> };

/** The block registry. */
export const blockRegistry: BlockRegistry = {
  meta: { type: 'meta', schema: blockSchemas.meta },
  callout: { type: 'callout', schema: blockSchemas.callout },
  table: { type: 'table', schema: blockSchemas.table },
  sequence: { type: 'sequence', schema: blockSchemas.sequence },
  erd: { type: 'erd', schema: blockSchemas.erd },
  userstory: {
    type: 'userstory',
    schema: blockSchemas.userstory,
    extractRefs: (data) => {
      const links = data.links;
      if (!Array.isArray(links)) return [];
      const refs: string[] = [];
      for (const link of links) {
        if (typeof link.ref === 'string' && link.ref.length > 0) refs.push(link.ref);
      }
      return refs;
    },
  },
  timeline: { type: 'timeline', schema: blockSchemas.timeline },
  kanban: { type: 'kanban', schema: blockSchemas.kanban },
  prose: { type: 'prose', schema: blockSchemas.prose },
  glossary: { type: 'glossary', schema: blockSchemas.glossary },
  proscons: { type: 'proscons', schema: blockSchemas.proscons },
  cvt: { type: 'cvt', schema: blockSchemas.cvt },
  stats: { type: 'stats', schema: blockSchemas.stats },
  code: { type: 'code', schema: blockSchemas.code },
  agenda: { type: 'agenda', schema: blockSchemas.agenda },
  tree: { type: 'tree', schema: blockSchemas.tree },
  pyramid: { type: 'pyramid', schema: blockSchemas.pyramid },
  flow: { type: 'flow', schema: blockSchemas.flow },
  state: { type: 'state', schema: blockSchemas.state },
  dfd: { type: 'dfd', schema: blockSchemas.dfd },
  journey: { type: 'journey', schema: blockSchemas.journey },
  gantt: { type: 'gantt', schema: blockSchemas.gantt },
  graph: { type: 'graph', schema: blockSchemas.graph },
  quadrant: { type: 'quadrant', schema: blockSchemas.quadrant },
  swimlane: { type: 'swimlane', schema: blockSchemas.swimlane },
  c4: { type: 'c4', schema: blockSchemas.c4 },
  uml: { type: 'uml', schema: blockSchemas.uml },
  frontend: { type: 'frontend', schema: blockSchemas.frontend },
  cluster: { type: 'cluster', schema: blockSchemas.cluster },
  block: { type: 'block', schema: blockSchemas.block },
  felogic: { type: 'felogic', schema: blockSchemas.felogic },
  wireframe: { type: 'wireframe', schema: blockSchemas.wireframe },
  endpoint: { type: 'endpoint', schema: blockSchemas.endpoint },
  pullquote: { type: 'pullquote', schema: blockSchemas.pullquote },
  layers: { type: 'layers', schema: blockSchemas.layers },
  matrix: { type: 'matrix', schema: blockSchemas.matrix },
  anatomy: { type: 'anatomy', schema: blockSchemas.anatomy },
  composition: { type: 'composition', schema: blockSchemas.composition },
  drivers: { type: 'drivers', schema: blockSchemas.drivers },
  options: { type: 'options', schema: blockSchemas.options },
  spec: { type: 'spec', schema: blockSchemas.spec },
  list: { type: 'list', schema: blockSchemas.list },
  stories: {
    type: 'stories',
    schema: blockSchemas.stories,
    extractRefs: (data) => {
      const refs: string[] = [];
      for (const story of data.items) {
        if (!Array.isArray(story.links)) continue;
        for (const link of story.links) {
          if (typeof link.ref === 'string' && link.ref.length > 0) refs.push(link.ref);
        }
      }
      return refs;
    },
  },
  pattern: { type: 'pattern', schema: blockSchemas.pattern },
  gallery: { type: 'gallery', schema: blockSchemas.gallery },
  chart: { type: 'chart', schema: blockSchemas.chart },
  figure: { type: 'figure', schema: blockSchemas.figure },
  steps: { type: 'steps', schema: blockSchemas.steps },
  faq: { type: 'faq', schema: blockSchemas.faq },
  envelope: { type: 'envelope', schema: blockSchemas.envelope },
  slo: { type: 'slo', schema: blockSchemas.slo },
  swot: { type: 'swot', schema: blockSchemas.swot },
  okr: { type: 'okr', schema: blockSchemas.okr },
  persona: { type: 'persona', schema: blockSchemas.persona },
  changelog: { type: 'changelog', schema: blockSchemas.changelog },
  team: { type: 'team', schema: blockSchemas.team },
  heatmap: { type: 'heatmap', schema: blockSchemas.heatmap },
  scorecard: { type: 'scorecard', schema: blockSchemas.scorecard },
  risk: { type: 'risk', schema: blockSchemas.risk },
  palette: { type: 'palette', schema: blockSchemas.palette },
  typescale: { type: 'typescale', schema: blockSchemas.typescale },
  dodont: { type: 'dodont', schema: blockSchemas.dodont },
  inventory: { type: 'inventory', schema: blockSchemas.inventory },
  array: { type: 'array', schema: blockSchemas.array },
  linkedlist: { type: 'linkedlist', schema: blockSchemas.linkedlist },
  bintree: { type: 'bintree', schema: blockSchemas.bintree },
  hashmap: { type: 'hashmap', schema: blockSchemas.hashmap },
  agentloop: { type: 'agentloop', schema: blockSchemas.agentloop },
  trace: { type: 'trace', schema: blockSchemas.trace },
  prompt: { type: 'prompt', schema: blockSchemas.prompt },
  context: { type: 'context', schema: blockSchemas.context },
  archmap: { type: 'archmap', schema: blockSchemas.archmap },
  divider: { type: 'divider', schema: blockSchemas.divider },
  bignumber: { type: 'bignumber', schema: blockSchemas.bignumber },
  takeaways: { type: 'takeaways', schema: blockSchemas.takeaways },
  statustable: { type: 'statustable', schema: blockSchemas.statustable },
  cycle: { type: 'cycle', schema: blockSchemas.cycle },
  benchmark: { type: 'benchmark', schema: blockSchemas.benchmark },
  sankey: { type: 'sankey', schema: blockSchemas.sankey },
  gitgraph: { type: 'gitgraph', schema: blockSchemas.gitgraph },
  treemap: { type: 'treemap', schema: blockSchemas.treemap },
  packet: { type: 'packet', schema: blockSchemas.packet },
  venn: { type: 'venn', schema: blockSchemas.venn },
  wardley: { type: 'wardley', schema: blockSchemas.wardley },
  harvey: { type: 'harvey', schema: blockSchemas.harvey },
  scqa: { type: 'scqa', schema: blockSchemas.scqa },
  scenarios: { type: 'scenarios', schema: blockSchemas.scenarios },
  fishbone: { type: 'fishbone', schema: blockSchemas.fishbone },
  storymap: { type: 'storymap', schema: blockSchemas.storymap },
  eventcontract: { type: 'eventcontract', schema: blockSchemas.eventcontract },
  saga: { type: 'saga', schema: blockSchemas.saga },
  slopegraph: { type: 'slopegraph', schema: blockSchemas.slopegraph },
  spans: { type: 'spans', schema: blockSchemas.spans },
  rollout: { type: 'rollout', schema: blockSchemas.rollout },
  neuralnet: { type: 'neuralnet', schema: blockSchemas.neuralnet },
  modelcard: { type: 'modelcard', schema: blockSchemas.modelcard },
  mindmap: { type: 'mindmap', schema: blockSchemas.mindmap },
  audit: { type: 'audit', schema: blockSchemas.audit },
  checklist: { type: 'checklist', schema: blockSchemas.checklist },
  perfbudget: { type: 'perfbudget', schema: blockSchemas.perfbudget },
  percentiles: { type: 'percentiles', schema: blockSchemas.percentiles },
  usecase: { type: 'usecase', schema: blockSchemas.usecase },
  pkg: { type: 'pkg', schema: blockSchemas.pkg },
  timing: { type: 'timing', schema: blockSchemas.timing },
  threatmodel: { type: 'threatmodel', schema: blockSchemas.threatmodel },
  chevrons: { type: 'chevrons', schema: blockSchemas.chevrons },
  roadmap: { type: 'roadmap', schema: blockSchemas.roadmap },
};
