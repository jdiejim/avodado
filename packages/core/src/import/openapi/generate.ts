...
import { isNearDuplicateTitle } from '../../validate.js';
import {
  HTTP_METHODS,
  type HttpMethod,
...
function renderEndpointSequence(slug: string, ep: Endpoint): string {
  ...
  const heading = `${ep.method.toUpperCase()} ${ep.path}`;
  const title = (ep.op.summary ?? heading).trim();
  ...
  const lines: string[] = [
    '```sequence',
    `id: ${id}`,
    ...(isNearDuplicateTitle(heading, title) ? [] : [`title: ${yamlString(title)}`]),
  ];
