...
const DECK_JS = `(function(){
  ...
  function fit(slide){
    ...
    // Scale to fit BOTH axes; allow enlarging small content (cap ~2.4x) so a lone
    // code/list/callout block fills the slide instead of floating tiny.
    var s=Math.min(2.4, content.clientWidth/r.width, content.clientHeight/r.height)*0.98;
