...
/* Animated gradient rail down the left edge. */
.docskin.slide::before{content:"";position:absolute;left:0;top:0;bottom:0;width:12px;
  background:linear-gradient(180deg,var(--navy),var(--purple),var(--teal),var(--blue),var(--highlight),var(--navy));
  background-size:100% 320%;animation:avo-rail 7s ease-in-out infinite;}
@keyframes avo-rail{0%{background-position:0 0;}50%{background-position:0 100%;}100%{background-position:0 0;}}
@media (prefers-reduced-motion:reduce){.docskin.slide::before{animation:none;}}
...
/* Content centers vertically when it fits, scrolls from the top when it doesn't
   (margin:auto on a flex child is the scroll-safe way to center). */
.slide-content{flex:1 1 auto;min-height:0;overflow:auto;display:flex;}
.slide-inner{margin:auto;width:100%;}
...
export function toSlides(doc: Document, opts: RenderPartsOptions = {}): string {
  ...
  const slideEls = slides
    .map((sl, i) => {
      ...
      return `<div class="docskin slide${isCover ? ' slide-cover' : ''}">${header}<div class="slide-content"><div class="slide-inner">${sl.html}</div></div></div>`;
    })
    .join('');
