...
/* All slides are the same 16:9 stage, sized to fit the viewport, and centered. */
.docskin.slide{display:none;width:min(94vw, calc((100vh - 116px) * 16 / 9), 1120px);aspect-ratio:16/9;margin:0 auto;
  padding:30px 44px;background:var(--white);border:1px solid var(--rule);border-left:12px solid var(--navy);
  border-radius:12px;box-shadow:0 14px 46px rgba(0,0,0,.14);overflow:auto;}
.docskin.slide.active{display:flex;}
.slide-content{margin:auto;width:100%;}            /* center content; scroll if it overflows */
.docskin.slide .section-block{margin:0;width:100%;}
.docskin.slide svg{max-width:100%;height:auto;}    /* diagrams scale to fit the slide */
.docskin.slide .diagram{overflow-x:auto;}
.docskin.slide img{max-width:100%;}
...
export function toSlides(doc: Document, opts: RenderPartsOptions = {}): string {
  ...
  const slideEls = slides
    .map((sl, i) => {
      ...
      return `<div class="docskin slide" style="border-left-color:${accent}"><div class="slide-content">${sl.html}</div></div>`;
    })
    .join('');
