...
avo check --json                # machine-readable: every diagnostic with its code, file, line, value
avo check --strict-prose        # prose warnings become errors
avo block <type>                # the fields, enums, terse forms, and example for one block
avo html docs/orders-api.md -p  # render and open it (slides … -p for a deck)
...
Prefix every command with `npx -y avodado` when `avo` is not on PATH.
...
| `E_SCHEMA` | A field is missing, wrong-typed, or unknown; the message contains the path. | Compare against `avo block <type>`. Do not add undocumented fields — the schema is strict. |
...
| `E_UNKNOWN_BLOCK` | A segment claims an unknown block type (rare — unknown fences normally fall through to plain code). | Use exactly one of the types `avo block` lists. |
