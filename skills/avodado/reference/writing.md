...
Read this file when a YAML question is not answered by `avo block <type>`.
...
for every block come from `npx -y avodado block <type>`.
...
- The info-string is exactly one of the block types listed by `avo block`
