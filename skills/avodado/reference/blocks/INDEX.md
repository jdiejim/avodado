...
Part of the **avodado** skill (the hub is `SKILL.md`, two folders up). One
line per block, mapped to the family file that holds its selection guidance.
For a block's fields and a validating example, run `npx -y avodado block
<type>`; bare `avo block` prints this same list. Twelve old block names
