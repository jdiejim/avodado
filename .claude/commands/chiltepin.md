---
description: Chiltepin docs — audit the repo for doc gaps, author one doc, or validate all docs.
---

# /chiltepin — Chiltepin documentation
...
```
/chiltepin audit [path] [--full]   audit the codebase; pick docs from a menu; author them
/chiltepin doc <target>            author one doc about a path, module, or feature
/chiltepin check                   validate all docs; fix errors; explain open warnings
```
...
Follow the chiltepin skill's seven-step selection procedure for each doc.
...
Run `chiltepin check` on everything you wrote. Fix every diagnostic.
...
2. Author one doc about the target. Follow the chiltepin skill's
   seven-step selection procedure.
3. Run `chiltepin check` before you finish. Fix every diagnostic.

## check

1. Run `chiltepin check`.
2. Fix every error.
