...
```timeline
items:
  - "[current] now · P0 — Core · parser + resolver"
  - "[next] next · P1 — CLI · init / check / render"
  - Later — studio
```
`status` is `done | current | next | future` (colours the dot). The object
...
```
Free cells render under the `columns` headers (default `Task` · `Update`); a
