...
description: >-
  ...
  archmap · block · endpoint · kanban · stats · divider — and 60 more, mapped in
  ...
  reference/blocks/contract.md (the exact field contract for all 76 blocks),
...
*Block glossary* below (`avo catalog` prints all 76 with descriptions). Casting
rules:
...
| File | Read it when |
|---|---|
| `reference/blocks/INDEX.md` | To find which family file documents a block — the map of all 76 types → family file, one line each, plus the alias table (old names → canonical). |
...
| `reference/blocks/contract.md` | The strict at-a-glance field contract for all 76 blocks — required vs optional fields, enums, number rules. |
...
types, and don't reach for all 76. Start from the doc's job and pick a stack:
...
- The info-string is exactly one of the **76 block types** catalogued in the
...
| `E_UNKNOWN_BLOCK` | A segment claims a block type the registry doesn't know. Defensive — you should rarely see it (unknown fences normally fall through to plain code, and near-miss typos surface as `W_SUSPECT_BLOCK`). | Make the fence info-string exactly one of the 76 documented types (`reference/blocks/INDEX.md`). |
