...
Chiltepin treats the files on disk as the only source of truth. The CLI, the coding
...
This very document is written in that format and rendered by the `chiltepin` renderer.

```callout
tone: note
title: Build discipline
body: Each phase ships complete before the next begins. A working chiltepin check and a
...
items:
  ...
  - label: P1 — CLI
    date: next
    status: next
    desc: chiltepin init / dev / check / new
  - label: P2 — Playground
    date: next
    status: next
  ...
  - label: P3 — MCP + CI
    date: later
    status: future
    desc: chiltepin MCP server + PR validation and visual diff
...
columns:
  - label: Now
    cards:
      ...
      - { title: chiltepin check, tag: validate + resolve refs }
      - { title: chiltepin init scaffold }
  - label: Next
    cards:
      - { title: chiltepin dev, tag: watch + hot reload }
      - { title: chiltepin new, tag: templates + TUI picker }
      - { title: Hosted playground + share links }
      - { title: chiltepin MCP server }
  - label: Later
    cards:
      - { title: PR visual diff in CI }
      - { title: chiltepin theme, tag: brand all blocks }
...
items:
  ...
  - { task: chiltepin check command, status: todo, priority: high }
