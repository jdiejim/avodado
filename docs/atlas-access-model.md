...
```table
columns: [Option, How it works, Edge case, Verdict]
rows:
  - ["1 · App-managed roles", "Roles assigned in our own admin panel and DB; Entra only signs in", "Trivial — two DB rows; governance is the problem, not the model", "✗ Rejected — nothing goes through SailPoint"]
  - ["2 · Hybrid", "Site groups via SailPoint; roles per site in our admin panel", "Trivial in the DB — but every role decision lives outside the IGA", "~ Viable — most scalable; roles ungoverned"]
  - ["3 · Global role groups (S+R)", "One global group per persona; the role applies at every site the user can access", "CM at Site A means CM at Site B too — must be subtracted via DB deny override", "~ Viable — kept as the fallback"]
  - ["4 · Per-site role groups (S×R)", "One group per persona per site; site and role bound together", "Native: SiteA-CycleManager + SiteB-Users — nothing to subtract", "✓ Chosen — matches the constraints best"]
```
...
```table
columns: [Persona, WP (wp:*), SC (scope:*), SS (schedule:*)]
rows:
  - ["Viewer (base — no group)", Read, Read, Read]
  - ["Supervisor", "Full (TBD)", "—", "—"]
  - ["Planner", "Edit (TBD)", "—", "—"]
  - ["Cycle Manager", "—", "Full (TBD)", "Full (TBD)"]
  - ["Cycle Planner", "—", "Edit (TBD)", "Edit (TBD)"]
  - ["Work Control Manager", "—", "Manage (TBD)", "Manage (TBD)"]
  - ["Work Week Manager", "—", "Manage (TBD)", "Manage (TBD)"]
  - ["Scheduler", "—", "—", "Edit (TBD)"]
```
