```meta
title: Meridian Access Model
subtitle: Role-based access for Meridian in three layers — per-site role groups requested through IGA, provisioned into IdP, with app-owned permissions.
tag: ACCESS MODEL · PER-SITE v1.0
```

## What we need

Meridian needs role-based access across the Meridian apps, with access requested through
...
items:
  ...
  - { title: IGA governs access, body: "IGA requests, approves, and provisions sites and roles; we never assign access ourselves.", tag: "WHO: role groups", icon: shield, accent: blue }
...
```options
items:
  - { kicker: "Option 1", title: App-managed roles, how: "Roles assigned in our own admin panel and DB; IdP only signs in.", pros: [Full control in our hands], cons: ["IGA controls only SSO, not access", Second source of truth, Custom tooling to govern and audit], verdict: "REJECTED — nothing goes through IGA", tone: rejected }
  - { kicker: "Option 2 · Hybrid", title: "Sites via IGA, roles in our app", how: "Site groups via IGA; roles assigned per site in our admin panel.", pros: [Most scalable — only site groups in IdP, "New roles are cheap — a DB change"], cons: ["Role grants bypass IGA governance", Audit splits across two systems], verdict: "VIABLE — most scalable; roles ungoverned", tone: warn }
  - { kicker: "Option 3 · S+R", title: Global role groups, how: "Site groups for read; one global role group applied at every site the user can access.", pros: [Fewest groups, Scales linearly, Fully governed by IGA], cons: ["A role applies at every site", "No way to scope a role to one site"], verdict: "VIABLE — kept as the fallback", tone: viable }
...
Meridian apps mount one shared middleware, and a single backbone owns *what*.
...
layers:
  ...
  - { label: Meridian apps — one model }
  - { label: Shared backbone }
nodes:
  - { id: sp, layer: 0, kind: external, name: IGA, tech: IGA — request / approve }
  - { id: idp, layer: 0, kind: identity, name: IdP, tech: "groups[] on the JWT" }
...
edges:
  - { from: sp, to: idp, label: provisions groups }
  - { from: idp, to: sc, label: "JWT" }
  - { from: idp, to: wp }
  - { from: idp, to: ss }
  - { from: idp, to: orch }
  - { from: sc, to: lib }
...
rows:
  - ["L1 — Identity (the engine)", "Are you a real, signed-in Meridian user?", "Meridian SSO on IdP, entirely from the token", "None — membership in any known group passes"]
...
gates:
  - { kicker: "L1 · Identity", label: Valid token, desc: "A valid JWT whose groups intersect a known, provisioned set.", source: "Source: IdP JWT" }
...
nodes:
  ...
  - { id: backend, col: 4, row: 1, kind: external, name: Meridian API, note: "same L1·L2·L3 gates" }
edges:
...
messages:
  ...
  - { from: sp, to: idp, label: "+ Site3-CycleManager" }
  - { from: idp, to: token, label: "groups: [guid-s3-cyclemgr]" }
...
items:
  ...
  - { label: "Phase 3 — Per-site groups", date: Later, status: future, desc: "Remaining plants onboarded — the script creates each plant's group set and IGA roles in one run; site enforcement becomes real and self-service opens." }
```
...
items:
  ...
  - { task: "Confirm the IGA flow — role→group mapping, per-plant reviewers, certification cadence (owner: IT/IGA)", status: todo, priority: high }
...
columns:
  - label: Now
    cards:
      - { title: "PLAT-01 · Plant-onboarding script + pilot plant groups + IGA flow" }
