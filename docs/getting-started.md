...
rows:
  ...
  - ["avo serve", "Serve the docs site locally with live reload — rebuilds + reloads on save"]
  - ["avo build", "Build a static HTML site from all docs — index, sidebar nav, cross-doc links"]
...
```stats
stats:
  - { value: "87", label: Block types, trend: flat }
