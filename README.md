...
`avo init` also drops in:

- `.avodado/skill/SKILL.md` — the authoring skill (block grammar + worked examples for all 41 blocks)
- `CLAUDE.md` — pointer for Claude Code so it follows the skill in `docs/`
- `.cursor/rules/avodado.mdc` — pointer for Cursor

…so any AI agent in your repo can author Avodado docs immediately.
...
| Theme | Look |
|---|---|
| `minimal` | Clean modern (default) — white, near-black ink, single blue accent, geometric sans |
...
| Package | Purpose |
| --- | --- |
| [`@avodado/core`](./packages/core) | Parser, Zod block schemas (all 41 types), validation, reference resolver. Pure (no I/O). |
