...
3. **Render** — `avo render` / `avo preview` turn a doc into a styled, self-contained HTML page (7 themes, inline SVG diagrams, no runtime).
...
Seven built-in themes — pass via `renderDocument(doc, { theme })` or `avo render --theme`, or set one with `avo theme`:
...
| Package | Purpose |
| --- | --- |
| [`@avodado/core`](./packages/core) | Parser, Zod block schemas (all 46 types), validation, reference resolver. Pure (no I/O). |
...
| [`@avodado/cli`](./packages/cli) | `avo` — `init / block / template / check / render / html / slides / pdf / export / preview / theme / sync` + per-tool installers. |
...
avo block list                        # list all 46 block types
...
avo slides docs/plan.md  [-p]         # one doc → slide deck  (one slide per section)
...
```
avo theme                       # interactive picker — built-ins + your saved themes
avo theme list                  # list built-in + saved custom themes
avo theme new sunset            # scaffold a new custom theme in .avodado/themes/
avo theme install ./my.theme.json   # validate any theme file and add it (--use to also activate)
avo theme use sunset            # make it the project default (shorthand: avo theme sunset)
avo theme use dark              # built-ins: textbook|minimal|soft|dark|teal|plum|slate
