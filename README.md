...
- `.avodado/skill/SKILL.md` — the authoring skill (block grammar + worked examples for all 52 blocks)
...
Six built-in themes — pass via `renderDocument(doc, { theme })` or `avo render --theme`, or set one with `avo theme`:
...
**Custom themes.** `avo theme new <name>` scaffolds `.avodado/themes/<name>.theme.json` — a small file where you pick a base theme and override any friendly color (`primary`, `accent`, `ink`, `paper`, …) or font slot (`display`, `body`, `mono`). Fill it in, then `avo theme` lists it alongside the built-ins so you can select it (or `avo theme use <name>` to activate it directly). Selecting copies it to the active `avodado.theme.json`; no rebuild — just re-render.
...
| Package | Purpose |
| --- | --- |
| [`@avodado/core`](./packages/core) | Parser, Zod block schemas (all 52 types), validation, reference resolver. Pure (no I/O). |
...
| [`@avodado/cli`](./packages/cli) | `avo` — `init / block / template / check / render / html / slides / pdf / export / preview / demo / catalog / design / prompt / skill / theme / sync` + per-tool installers. |
...
| `avo export [globs]` | Batch-export many docs (`--format html,slides,pdf --out dir`) |
| `avo demo [format]` | Render the built-in showcase of every block |
| `avo catalog [-s]` | A catalog of every block — identifier + what it does + a live sample |
| `avo design [slug]` | Design-pattern library (80: system · AI/agent · GoF) — grab a template; `--all` gallery |
...
| `avo theme [name]` | Pick / list / create / install the document theme |
...
avo block list                        # list all 52 block types
...
avo new --type erd --out docs/data.md # (older form of block/template scaffolding)
```
...
avo demo slides                       # …as a slide deck  (also: avo demo pdf)
avo catalog                           # a catalog of every block — identifier + what it does + a live sample
avo catalog -s                        # …one block per slide (great as an AI/teammate reference deck)
avo design                            # list design patterns (system design + AI/agent + GoF code)
...
avo design --all [--system|--ai|--code] [-s]   # render the pattern gallery (sample HTML / slides)
...
```
avo prompt                            # pick a ready-to-paste authoring prompt (ADR, roadmap, RBAC…)
avo prompt list                       # …or list them; avo prompt new <name> saves a custom one
...
avo cursor                            # …Cursor   ·  avo github → Copilot  ·  avo windsurf → Windsurf
```
...
avo theme list                  # list built-in + saved custom themes
avo theme new sunset            # scaffold a new custom theme in .avodado/themes/
avo theme install ./my.theme.json   # validate + add a theme; then asks to set it as default (--use skips the prompt)
avo theme use sunset            # make it the project default (shorthand: avo theme sunset)
avo theme use dark              # built-ins: textbook|minimal|soft|dark|teal|slate
avo theme custom                # scaffold a blank custom theme into avodado.theme.json
...
PDF export needs Chromium. `avo pdf` / `avo export --format pdf` **download it automatically on first use** (the matching build, ~100 MB, one time). To pre-install manually: `npx playwright install chromium`.
