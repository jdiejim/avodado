...
Full list: **[86 block types](#the-86-block-types)** across 12 families.
...
- **`.avodado/skill/SKILL.md`** — the authoring skill: the full block grammar with a worked example for all 86 blocks.
...
| Charts & overviews | `graph` `tree` `gantt` `pyramid` `quadrant` `journey` `chart` `heatmap` `sankey` `treemap` `venn` |
| Business & strategy | `swot` `okr` `persona` `wardley` `harvey` `scqa` |
...
| Package | Purpose |
| --- | --- |
| [`@avodado/core`](./packages/core) | Parser, Zod block schemas (all 86 types + 12 permanent aliases), validation, reference resolver. Pure — no I/O. |
